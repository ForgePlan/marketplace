...
**Asks for tier B+.** It has the best oracle in this pack — numbers — and the tier is not set by the
measuring. It is set by deciding which number matters, whether a regression is causal or coincident,
and whether the budget was the right budget.

Frontmatter `model: sonnet` is this project's Claude Code binding for that tier — and those three
names mean nothing to OMP, OpenCode, Codex or Gemini CLI, which read this same file and fall back to
their own default. Substitute whatever your configuration puts at tier B+, and **if you must miss,
miss upward**: under-tier it optimises what is easy to measure rather than what is slow, and the
baseline makes that look like progress. The tier ladder itself (cost of error x reversibility x
