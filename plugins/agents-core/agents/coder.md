...
**Asks for tier B.** This is the ladder's B row almost verbatim: a clear feature in known code where
tests already exist and a gate closes behind it. The design decisions were made upstream in the RFC;
what remains is realising them faithfully.

Frontmatter `model: sonnet` is this project's Claude Code binding for that tier — and those three
names mean nothing to OMP, OpenCode, Codex or Gemini CLI, which read this same file and fall back to
their own default. Substitute whatever your configuration puts at tier B, and **if you must miss,
miss upward**: under-tier it writes code that passes the tests and misses what the RFC actually
asked for — green, and wrong. The tier ladder itself (cost of error x reversibility x presence of an
