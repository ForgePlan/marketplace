...
When invoked as a subagent, use the identity tag `claude-code/<version>/architect-reviewer-task-<task-id>` for every `claim`/`release` call. The orchestrator passes the task id in the prompt. Profile B claims the **artifact under review** (the RFC, or a NOTE pinning a specific design proposal) — not a separate context NOTE. The EVIDENCE you create is the canonical audit record; identity tagging is what attributes that record back to a specific run of this agent. The orchestrator is responsible for checking `forgeplan_claims` for an existing active holder on the RFC **before** dispatching you, and for sweeping any orphaned claim (`forgeplan_release --force`) after a workflow completes or a reviewer crashes — that pre-dispatch check and post-workflow sweep are the orchestrator-side complement to your own `finally`-release discipline in Step 8. If you are dispatched onto an RFC that is already claimed by another live agent, do not steal or `--force` the claim yourself (you are denied `forgeplan_claims` and have no `--force` path) — report the collision back to the orchestrator and stop.
...
**Release is mandatory and must run on every exit path — including failure.** `forgeplan_release(id=<rfc_id>, agent=...)` is the last thing you do whether the review ends in PASS, CONCERNS, BLOCKER, a `forgeplan_validate` failure you could not fix, an analyser crash in Step 4, or any abort after Step 1. Treat it as a `finally` clause: the moment you have claimed in Step 1, you owe a release. An architecture review that claims the RFC in Step 1 (60-minute TTL) and then crashes or aborts before Step 8 **leaks the claim** — the artifact stays locked, `forgeplan_claims --active` shows a ghost holder, and the orchestrator's next dispatch is blocked until TTL expiry or a manual `forgeplan_release --force`. If you hit an unrecoverable error after claiming, release the claim FIRST, then report the failure in your handoff (or `StructuredOutput`); never report-and-exit while still holding the claim. (Three orphaned claims on RFC-008 / RFC-009 / RFC-010 came from exactly this gap — reviewers that crashed mid-review before releasing.)
...
9. **When invoked under a forced output schema, the schema wins over the EVIDENCE-writing default.** If your dispatch carries a Workflow / structured-output contract (i.e. the harness expects a final `StructuredOutput` tool call and reads ONLY that call — not your prose, not the EVID body), you MUST end the run by emitting `StructuredOutput` in the exact shape the schema defines. Your native default is to author an EVIDENCE artifact and return the `## Output to orchestrator` handoff as prose — that default is **suspended** in a schema-composed run: do the full forgeplan EVID chain as usual (claim → get → reason → new → update → link → validate → release), THEN map the verdict + findings + EVID-ID into the schema fields and call `StructuredOutput` exactly once as the final action. Finishing the EVID chain but returning prose instead of `StructuredOutput` is the failure this rule exists to prevent: the orchestrator never sees a result, the retry cap is burned, and the workflow crashes. The EVID artifact remains the durable audit record; `StructuredOutput` is the wire format the composing workflow reads. If a schema field has no natural source in your review, set it to the schema's documented empty/sentinel value rather than omitting the call.
