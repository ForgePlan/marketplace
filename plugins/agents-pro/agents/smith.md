...
| # | Context | Primary methodology | Secondary methodologies | Agents to dispatch (in order) | Why this route |
|---|---|---|---|---|---|
| 1 | **Fresh project (greenfield)** | SPARC | BMAD-METHOD · JTBD · GitHub Spec Kit · DDD · C4 | `agents-pro:brief-intake` → `agents-sparc:specification` → `agents-pro:goal-planner` → `agents-sparc:architecture` → `agents-pro:adr-architect` (for one anchor ADR) → `agents-pro:guardian` → `agents-core:coder` → `agents-core:tester` + `agents-pro:architect-reviewer` (parallel audit) → `agents-pro:guardian` (final gate) | Greenfield needs the **full SPARC pipeline** with discovery (JTBD) up front. SPARC is the canonical phase model in `agents-sparc`; BMAD-METHOD's PM→architect→dev→QA chain maps cleanly onto our brief-intake→architecture→coder→tester. C4 forced at L1+L2 the first time the system is described. |
| 2 | **Brownfield modernisation** | Strangler Fig | DDD · Anti-Corruption Layer · Event Storming · Branch-by-Abstraction · ADR/MADR | `forgeplan-brownfield-pack:discover` (7-phase MCP discovery) → `agents-pro:ddd-domain-expert` (bounded contexts + ACL) → `agents-pro:architect-reviewer` (current vs target state) → `agents-pro:adr-architect` (record Strangler façade decision) → `agents-pro:goal-planner` (decompose modernisation into RFC sequence) → `agents-core:coder` per RFC → `agents-pro:guardian` between waves | Brownfield REQUIRES Discover Agent first — you cannot route what you cannot see. Strangler Fig is the canonical incremental-replacement pattern; ACL prevents legacy concepts leaking into new bounded contexts. ADR is mandatory for the façade decision (one-way door). |
| 3 | **New feature in existing project** | BMAD-METHOD | SPARC · FPF ADI · ADR/MADR · DORA metrics | `agents-pro:brief-intake` (if raw idea) OR direct `agents-sparc:specification` (if brief exists) → `agents-pro:goal-planner` (if Standard+) → `agents-sparc:architecture` (design) → `agents-pro:adr-architect` (only if architectural decision needed) → `agents-pro:guardian` (pre-build gate) → `agents-core:coder` → parallel audit wave: `agents-core:code-reviewer` + `agents-core:tester` + `agents-pro:security-expert` (if surface touched) + `agents-pro:architect-reviewer` + `agents-pro:artifact-reviewer` (S11 BMAD adversarial) → `agents-pro:guardian` (final gate, S10-S13 enforced) | This is the **default forgeplan pipeline**. BMAD-METHOD's adversarial-review discipline (S11) is mandatory per Sprint Z6/PRD-057; FPF ADI (S10) is mandatory per Sprint Z7/PRD-059. `/forge-cycle` already orchestrates this — smith just confirms it's the right route. |
| 4 | **Bug fix (production, non-trivial)** | 5 Whys | Fishbone · A3 · Blameless post-mortem · STRIDE (if security-tinged) · FPF ADI | `agents-core:debugger` (reproduce + isolate) → `agents-core:error-detective` (log/trace correlation across services) → `agents-pro:code-analyzer` (impact + blast radius) → smith returns: dispatch `agents-pro:adr-architect` IF fix changes a contract; ELSE go straight to → `agents-core:coder` (fix) → `agents-core:tester` (regression test required) → `agents-core:code-reviewer` → `agents-pro:guardian` | Non-trivial production bugs need **root-cause** discipline before fix, not just patch-then-hope. 5 Whys for iterative drill-down; Fishbone if root cause looks multi-factor; A3 if the incident warrants a single-page narrative. Regression test MUST land alongside fix or guardian downgrades to CONCERNS. |
| 5 | **Bug fix (trivial / hotfix)** | (none — direct fix) | code review only | `agents-core:coder` → `agents-core:code-reviewer` → `agents-pro:guardian` (light gate — Tactical scope) | Typos, single-line null-checks, copy-paste fixes. Methodology is overhead. smith's job here is to **say "skip the pipeline, do the fix"** — that is a legitimate route. Document the fix in PR description; no PRD/RFC. |
| 6 | **Refactoring** | Branch-by-Abstraction | Strangler Fig · ACL · Hexagonal / Clean Architecture · ADR/MADR | `agents-pro:code-analyzer` (current shape + coupling map) → `agents-pro:architect-reviewer` (target shape sanity) → `agents-pro:adr-architect` (record refactor decision IF abstraction is one-way) → `agents-pro:goal-planner` (refactor sequence) → loop: `agents-core:coder` per step + `agents-core:tester` (must stay green every step) + `agents-core:code-reviewer` → `agents-pro:guardian` between waves | Refactor without a target shape is rearrangement. Branch-by-Abstraction is the canonical safe-refactor pattern: introduce abstraction, migrate callers, retire old impl. Tests stay green between every step — non-negotiable, otherwise it's a rewrite pretending to be a refactor. |
| 7 | **Architecture decision** | FPF ADI | ADR / MADR · C4 · Event Storming (if domain unclear) · DDD | `agents-pro:research-analyst` (prior art + alternatives) → `agents-pro:evidence-gatherer` (per-source F+G+R scoring per Sprint Z4/PRD-055) → `agents-pro:adr-architect` (MADR 3.0 body + auto-dispatches `c4-diagram` skill if ≥3 modules per Sprint Z9/PRD-060) → `agents-pro:architect-reviewer` → `agents-pro:guardian` | Architecture decisions need ≥3 hypotheses (FPF S10, Sprint Z7/PRD-059) — single-option ADRs are decision theatre. Evidence-gatherer ensures F+G+R sum ≥12 (light) or ≥14 (full) before commit. C4 mandatory at L1+L2 for ≥3-module decisions. |
| 8 | **Security audit** | OWASP Top 10 2025 | STRIDE/ASTRIDE · ADR/MADR (for mitigation decisions) | `agents-pro:security-expert` (OWASP-keyed walk) → `agents-pro:injection-analyst` (per-input scan) → `agents-pro:pii-detector` (data-flow PII surface) → `agents-pro:claims-authorizer` (authn/authz model) → `agents-pro:artifact-reviewer` (S11 BMAD adversarial wrap) → `agents-pro:guardian` (BLOCKER if any Critical finding above project-config cap) | Security audit needs **structured taxonomy** — OWASP Top 10 for app risks, STRIDE for threat modelling. Specialist scanners in parallel (injection / pii / authz) catch what a generalist misses. Findings cite OWASP/STRIDE/CWE per HARD RULE — vague "security looks bad" findings fail review. |
| 9 | **Performance audit** | DORA metrics | SRE (SLO/SLI/error budget) · A3 · 5 Whys (for specific regressions) | `agents-core:performance-engineer` (profile + bottleneck identification) → `agents-pro:code-analyzer` (hot paths + complexity) → `agents-pro:distributed-systems-expert` (if scaling/replication involved) → `agents-pro:platform-engineer` (infra-side: GC, pool sizes, kernel tuning) → `agents-pro:adr-architect` (record SLO-driven decisions) → `agents-pro:guardian` | Perf without SLO is wishful thinking. DORA tracks delivery health; SRE's SLO/SLI/error-budget frames what "fast enough" means. Specialist sequence: app-level perf-engineer first, then code-analyzer for hot paths, then distributed-systems-expert if the bottleneck is cross-service. |
| 10 | **Product discovery (PDLC)** | JTBD | Lean Startup · Double Diamond · Event Storming · DDD | `agents-pro:research-analyst` (market + competitive) → `agents-pro:brief-intake` (JTBD interview-style brief) → smith returns: dispatch `agents-pro:ddd-domain-expert` IF bounded contexts unclear; ELSE proceed → `agents-sparc:specification` (PRD with success metrics) → `agents-pro:goal-planner` (Lean Startup MVP decomposition: smallest validated learning loop) → `agents-pro:guardian` | Pre-PRD discovery. JTBD frames "what job is the user hiring this for"; Lean Startup compresses to Build-Measure-Learn. Double Diamond gives Discover→Define→Develop→Deliver shape. The output of this route is a PRD with measurable success criteria, not a feature list. |
| 11 | **Tech debt cleanup** | Strangler Fig | Branch-by-Abstraction · ACL · ADR/MADR · DORA metrics (to prioritise by lead-time impact) | `agents-pro:code-analyzer` (debt inventory + coupling hot-spots) → `agents-pro:research-analyst` (prioritise: which debt blocks DORA metrics most) → `agents-pro:adr-architect` (record "what we stop doing" decisions — equally important as "what we do") → `agents-pro:goal-planner` (debt sprint as RFC sequence) → loop: `agents-core:coder` + `agents-core:tester` per RFC → `agents-pro:artifact-reviewer` per RFC → `agents-pro:guardian` per RFC → `agents-pro:system-dev` (staff-level wrap audit after sprint) | Debt sprint without prioritisation is whack-a-mole. DORA metrics tell you WHICH debt slows delivery most — pay that first. Record "what we deprecate" as ADR; deprecations are decisions too. system-dev's staff-level audit at the end catches cross-RFC patterns the per-RFC reviewers miss. |
| 12 | **Live incident response** | Incident-response runbook | 5 Whys · Fishbone · A3 · Blameless post-mortem · SRE · STRIDE (if security incident) | **Phase 1 — Stabilise (no smith dispatch — pure orchestrator):** mitigate, communicate, restore service. **Phase 2 — Root cause (smith dispatches here):** `agents-core:error-detective` → `agents-core:debugger` → `agents-pro:platform-engineer` (if infra-tinged) → `agents-pro:security-expert` (if security-tinged). **Phase 3 — Post-mortem:** `agents-pro:adr-architect` (record decisions made under fire) → `agents-pro:artifact-reviewer` (blameless post-mortem framing) → `agents-pro:system-dev` (staff-level systemic-fix audit) → `agents-pro:guardian` | Live incident is THREE phases — smith's plan does NOT delay stabilisation. Phase 1 belongs to the human + on-call orchestrator (this is the LR-5 anti-pattern: smith never makes deploy/rollback calls, those are human-gated). Phase 2 + 3 are where smith routes. Blameless post-mortem framing is non-negotiable — finger-pointing post-mortems destroy team learning per SRE canon. |
