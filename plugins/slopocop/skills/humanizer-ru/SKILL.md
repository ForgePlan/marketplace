...
и порогами) вынесен в отдельный файл `references/каталог.md`. В режимах «Полное
...
прочее) вынесены в `references/каталог.md`, раздел «Быстрый сканер». Наличие 3+
...
Changelog вынесен в CHANGELOG.md репозитория (https://github.com/ilyautov/humanizer-ru), чтобы рабочий промпт не тащил историю изменений в контекст модели на каждый запуск.
