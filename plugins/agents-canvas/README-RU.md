...
Проведи **дизайн-систему из Pencil** до **нативного кода в целевом фреймворке проекта** с независимым
гейтом качества на каждой передаче. Фреймворк -- это вход, который резолвится на Step 0 (детект из
AGENTS.md/CLAUDE.md/package.json -> анонс или явный вопрос пользователю, если не удалось определить);
CANVAS генерирует код нативно в этом одном фреймворке. CANVAS -- собственное имя методологии
(Capture-Audit-Norm-check-Vectorize-Assemble[-Spread]) -- инстанс контракта суб-цикла AD/AID-PDLC
(ADR-010 / RFC-021): мастер (`canvas-coordinator`) ведёт пятифазную прогулку -- **C**apture, **A**udit,
**N**orm-check, **V**ectorize, **A**ssemble -- диспатча **каждую фазу и каждого верификатора через
`Task`**, так что генератор никогда не является верификатором, а fail-closed PreToolUse-хук блокирует
исходники дизайн-системы, пока не активирован токен-контракт (**hook-gate = Yes**).
...
> `agents-canvas` -- агенты `canvas-*` -- входной скилл `/canvas`. Топология: фреймворк -- вход, который
> резолвится на Step 0 (детект AGENTS.md/CLAUDE.md/package.json -> анонс/явный вопрос); CANVAS генерирует
> **нативно в этом одном фреймворке** проекта. Web Components/Lit -- один из выбираемых таргетов (канон,
> только если стек проекта -- Web Components), не канон по умолчанию. Fan-out на несколько фреймворков
> (тонкие обёртки React/Vue/Svelte/Angular/Solid, фаза Spread) убран из дефолтного конвейера --
> опционален, вне дефолта (будущий ADR-016). Токены: инструмент токенов проекта; **Style-Dictionary** --
> один из вариантов. Контракт единого `tokens.json` -> CSS custom properties -- инструмент-агностичен,
> никогда не форкается.

## Обновление с 0.3.0 → 1.0.0 (ЛОМАЮЩЕЕ)

**v1.0.0 делает CANVAS агностичным к стеку.** Он больше не генерирует по умолчанию Lit Web-Components-мастер + обёртки под фреймворки. Теперь он генерирует **нативно в том одном фреймворке, который объявлен в проекте** — резолв на Step 0 из `AGENTS.md` / `CLAUDE.md` / `package.json` (с анонсом или явным вопросом, если не найден). Lit/Web-Components и мульти-фреймворк-путь «Spread» стали **опциональными, вне дефолта** (ADR-016).

**Действие при обновлении: один раз прогнать `/canvas-init`.** Токен-гейт теперь выводит охраняемые пути под ваш резолвнутый фреймворк. На обновлённой установке со старыми `packages/**` globs гейт подставляет fail-SAFE catch-all (over-guard), пока вы не переинициализируетесь, — так что он никогда молча не открывается, — а `/canvas-init` восстанавливает точное покрытие под ваш реальный layout.
...
/canvas                                                # запустить прогулку C-A-N-V-A по срезу DS
```

`/canvas` отказывается стартовать без **источника дизайна** (путь к каноническому `.pen`-файлу + доступный
Pencil MCP) и резолвленного Step 0 фреймворка проекта (+ списка доп. фреймворков-таргетов, только если
запрошена опциональная фаза Spread). Нет дизайн-намерения? Greenfield -> `/bmad`, фича -> SPARC,
баг -> RIPER.
...
[C1 intake: задача «дизайн-система Pencil -> код»; координатор отказывается без источника дизайна + Pencil;
 Step 0 резолвит фреймворк проекта (детект AGENTS.md/CLAUDE.md/package.json -> анонс/явный вопрос)]
  |
  ...
  v  Assemble    canvas-coder             нативный код резолвленного фреймворка + stories + визуальные регресс-тесты
...
[ОПЦИОНАЛЬНО, вне дефолтного конвейера -- будущий ADR-016]
  ·  Spread      canvas-porter-framework  (x5 ПАРАЛЛЕЛЬНЫЙ fan-out -- по одному пакету на агента,
                                           непересекающиеся файлы, изоляция git-worktree)
                                          обёртки на доп. фреймворках (напр. React/Vue/Svelte/Angular/Solid)
                                          + parity-тесты
...
это разблокировка C5**: только тогда хук пропускает записи в исходники дизайн-системы. **Опциональная фаза
Spread** (вне дефолтного конвейера, будущий ADR-016), если задействована -- **единственный параллельный
fan-out**: по одному агенту на пакет фреймворка, строгое владение файлами, изоляция git-worktree, каждый
`blockedBy` PASS код-гейта (FR-9).
...
| `canvas-porter-storybook` | **V** Vectorize | creator-contract | Переносит утверждённую DS в токен-контракт (RFC) на инструменте токенов проекта (напр. Style-Dictionary) + story-спеки + референс-скриншоты + port-манифест. Обычный `Task`-саб-агент. |
| `canvas-coder` | **A** Assemble | C-coder | Строит Storybook (нативные компоненты резолвленного фреймворка проекта + `*.stories.ts` + визуальные регресс-тесты + токен-тема). |
...
| `canvas-porter-framework` | **S** Spread (опц.) | C-coder | Опционально (вне дефолтного конвейера, будущий ADR-016): портирует компоненты на доп. фреймворки (напр. React/Vue/Svelte/Angular/Solid) против общего токен- и story-контракта; parity-тесты. По одному агенту на пакет в параллельном fan-out. |
...
| Компонент | Описание |
|---|---|
| `/canvas` | Мастер-плейбук -- прогулка C-A-N-V-A (+ опциональная фаза Spread вне дефолта, будущий ADR-016), таблица ADR-010 C1-C6, обязательные C4-гейты, дисциплина диспатча FR-9, таблица «когда vs соседи». |
...
| `PreToolUse`-хук | `canvas-gate.sh` -- fail-closed; блокирует записи в дизайн-систему + код резолвленного фреймворка (плюс доп. пакеты при опциональной фазе Spread), пока токен-RFC не активен. |
...
- **context7** -- агенты, касающиеся кода (`canvas-coder`, `canvas-porter-storybook`,
  `canvas-porter-framework` [опционально, фаза Spread], `canvas-storybook-validator`, и `canvas-design`
  там, где он трогает Storybook/инструмент токенов проекта), **обязаны** использовать **context7 MCP**
  (`resolve-library-id` -> `query-docs`) для доков Storybook / резолвленного фреймворка проекта (Step 0) /
  инструмента токенов проекта (напр. Style-Dictionary) -- и, только если стек проекта Web Components, для
  Lit; при опциональной фазе Spread -- для доп. фреймворков-таргетов -- **до написания кода** и предлагать
  пользователю использовать context7 на любой вопрос о библиотеке/версии.

## Требования

- **MCP:** `pencil` (редактор `.pen` -- без него методология не работает).
- **MCP:** `context7` -- обязателен для доков резолвленного фреймворка проекта (Step 0) + Storybook +
  инструмента токенов проекта в код-фазах (см. «Дизайн-референсы» выше).
