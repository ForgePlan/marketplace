...
| **C3 — Phase agents** | Spec=`specification` → Pseudo=`pseudocode` → Arch=`adr-architect`+`architecture` → Refine=`refinement`+`coder` (optionally the TDD sub-cycle) → Completion retro=`evidence-recorder`. | agents-sparc / agents-pro / agents-core |
