...
OUTPUT_DIR = PROJECT_ROOT / "skills" / "fpf-knowledge" / "sections"
...
def split_spec():
    ...
    # Preserve applied-patterns (our additions, not from FPF spec)
    ap_dir = OUTPUT_DIR / "applied-patterns"
    ap_backup = None
    if ap_dir.exists():
        import tempfile
        ap_backup = Path(tempfile.mkdtemp()) / "applied-patterns"
        shutil.copytree(ap_dir, ap_backup)
    ...
    # Restore applied-patterns if backed up
    if ap_backup and ap_backup.exists():
        shutil.copytree(ap_backup, ap_dir)
        print(f"Restored applied-patterns/ ({len(list(ap_dir.iterdir()))} files)")
