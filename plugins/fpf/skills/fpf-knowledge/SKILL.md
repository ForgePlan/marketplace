...
New to FPF? Start here:

1. **Read the problem.** What are you actually trying to do?
2. **Pick a verb** from the router table below — it tells you where to go in the spec.
3. **Use a command** if you want guided, step-by-step help:
   - `/fpf-decompose` — break a system into parts
   - `/fpf-evaluate` — compare alternatives and score confidence
   - `/fpf-reason` — think through a question with the ADI cycle
4. **Read plain language first**, FPF terms second. The framework is a tool, not a religion.
...
Practical, step-by-step guides for the most common FPF workflows. These bridge
the gap between the formal specification and everyday use.

| Pattern | What it does | Guide | Command |
|---------|-------------|-------|---------|
| Decomposition | Break a system into bounded contexts with roles and interfaces | [decomposition-guide](sections/applied-patterns/decomposition-guide.md) | `/fpf-decompose` |
| Evaluation | Compare alternatives using F-G-R scoring and Trust Calculus | [evaluation-guide](sections/applied-patterns/evaluation-guide.md) | `/fpf-evaluate` |
| Reasoning | Think through a question using the ADI cycle | [reasoning-guide](sections/applied-patterns/reasoning-guide.md) | `/fpf-reason` |
| Forgeplan Integration | Map FPF outputs to forgeplan artefacts (PRD, RFC, ADR) | [forgeplan-integration](sections/applied-patterns/forgeplan-integration.md) | — |

### Forgeplan Integration (route → shape → reason → evidence)

When forgeplan is available, FPF outputs map directly to project artefacts:

1. **Route** — Use the thinking-verb router to find the right FPF section.
2. **Shape** — Apply decomposition, evaluation, or reasoning to produce structured output.
3. **Reason** — Capture the WHY in an ADR so the team can audit decisions later.
4. **Evidence** — Score confidence with Trust Calculus; log gaps so they become tasks.

| FPF output | Forgeplan artefact | Captures |
|------------|-------------------|----------|
| System decomposition | PRD | WHAT the system must do |
| Context design | RFC | HOW a part is built |
| Decision evaluation | ADR | WHY a choice was made |
| Confidence scores | Evidence log | TRUST level and gaps |
...
| AP | [Applied Patterns](sections/applied-patterns/_index.md) | 4 | **Practical guides**: decomposition, evaluation, reasoning, forgeplan integration. |
