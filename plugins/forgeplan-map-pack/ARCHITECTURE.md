...
- **Layer A** (GC-1..GC-4 + GC-7..GC-11) — structural + content-completeness checks over the document alone. Runs on any `map.json`, including the vendored smoke-test fixture. The v0.11.0 content gates (CM-13): **GC-7** every node has a valid ISO `found_at` (append-stability, BLOCKER), **GC-8** flow completeness — multi-node flows carry `edge_ids` + RU `steps` (WARN), **GC-9** layer-meta canon — a `scope:"layer"` doc has the frozen keys + `<parent>::<zone>` map_id + no auto-confirmed `needs_confirm` floor (BLOCKER), **GC-10** `is_mega ⟺ kind=="mega"` with non-empty children (BLOCKER), **GC-11** no grid-neighbour accent collision (WARN). Covered by `test/guardian-content-gates.mjs` (`node test/guardian-content-gates.mjs`).
