...
2b. **NEVER self-author the pipeline inputs — for a LAYER build either (findings B+C).** `.extract.json` (with top-level `repo_head` + `project`), `.edges.json` (`{typedLink,codeDep}`), and `.composition.json` are produced UPSTREAM — by `zone-extractor` and `edge-verifier` — for a scoped layer exactly as for the top map. If you are dispatched for a layer and those scoped inputs do not exist yet, that is an orchestration gap: say so and stop; do NOT hand-write them yourself. The live dogfood proved self-authored scoped inputs are unreliable (a self-authored layer forgot `repo_head` → a `nogit:` fingerprint). Your inputs are given; your output is flows + the script call. (A dispatch prompt that asks you to author scoped inputs is asking you past this rule — flag it, like any re-prompt-past-a-decline.)
