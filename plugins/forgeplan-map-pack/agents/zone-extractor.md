...
6. **Collapse any zone whose real member count exceeds 8** into exactly one `is_mega:true`/`collapsed:true` node referencing all members as `children` (originals stay in `nodes[]`) — see the skill's Algorithm 5, including the synthetic-provenance convention for the mega-node itself.
...
3. **Never** let a zone exceed 8 visible member nodes without collapsing to a mega-node — the threshold is unconditional and fixed at 8 in P1.
...
| Zone with 9+ nodes rendered flat, no mega-node | Check every zone's final member count before finalizing; collapse unconditionally past 8 |
