...
| `phase_mismatch` | artifact has status=active AND phase=shape/validate (early-cycle phase) AND r_eff>0 **AND the artifact is in scope for this session's work** (ADR-022 INV-3 — never a sweep of the historical backlog) | AUTO | Advance phase via `mcp__forgeplan__forgeplan_phase_advance` |
...
4. **phase_mismatch** — Common after sprint closure when `forgeplan_phase_advance` is omitted. An artifact can have `status=active` (correct) but `phase=shape` or `validate` (stale early-cycle value). The operation is reversible: `phase_advance` appends to the artifact's phase history, so the prior value survives in `.forgeplan/state/<id>.yaml` and one `--to <prior>` call restores it. The multi-signal gate (status=active AND early phase AND R_eff>0) prevents false positives on genuinely incomplete artifacts.

   **Reversible is not the same as free, and AUTO is bounded because of that** (ADR-022 DD-5 / INV-3). The marker records where an artifact actually stopped. Each advance keeps its own history row, but the aggregate reading — on 2026-09-02, 300 of 331 active artifacts had never left `validate` — is a measurement of how the pipeline really ran, and a bulk sweep erases it while every individual row stays intact. So AUTO may advance a marker only for an artifact being worked in the current session. Pointing it at the historical backlog to make `forgeplan_health` green is the exact move INV-3 forbids.
