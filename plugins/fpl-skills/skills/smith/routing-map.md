...
### Enforced-TDD sub-cycle

- One sentence: Build-stage sub-cycle that turns an active SPEC's `#### Scenario`s into a test plan, then failing tests (RED), which an **independent** validator certifies before the oracle is frozen and a **different** agent writes the code (GREEN) — with a fail-closed PreToolUse gate denying the implementer any test-file write.
- When it shines: Implementing an already-active, scenario-bearing SPEC where the expensive failure is an implementer quietly weakening a test until it passes; anywhere SPARC's Refinement or RIPER's Execute is test-critical and delegates here.
- When NOT to use: Design still open (no SPEC, or a SPEC with no `#### Scenario`s — do SPARC first); trivial hotfixes; exploratory spikes where nobody yet knows what to assert.
- Source: RFC-012 — instance #1 of the ADR-010 sub-cycle contract; `plugins/agents-tdd/` (`/tdd-init` once, then `/tdd`). Kent Beck, *Test-Driven Development: By Example* (2002).

### CANVAS

- One sentence: Master-coordinated design-system → code walk — **Capture → Audit → Norm-check → Vectorize → Assemble** — that turns a Pencil design into one `tokens.json` → CSS-custom-properties contract plus native components in the project's own framework, with a fail-closed hook blocking design-system source until the tokens RFC is active.
- When it shines: Porting an approved Pencil design system to tokens + Storybook + code, where the dominant failure is the single token contract forking the moment someone hardcodes a value before the tokens are agreed.
- When NOT to use: Feature logic (SPARC), greenfield products (BMAD), bug hunts (RIPER); a one-shot heuristic UX audit of existing UI (`/laws-of-ux:ux-review` alone); no approved design intent and no `.pen` file — CANVAS refuses.
- Source: RFC-021 + RFC-022 / ADR-015 (framework resolved as a Step-0 input) — instance #5 of the ADR-010 sub-cycle contract, warranted by the ADR-012 hook-gate test; `plugins/agents-canvas/` (`/canvas-init` once per branch, then `/canvas`).
...
- All Profile A/B/D agents enforce LR-8 lint (Sprint W): they must deny `Write`, `Edit`, `NotebookEdit` plus `forgeplan_activate` (and Profile B additionally denies `forgeplan_reason` and `memory_retain`). The claim-tool split runs the OTHER way than an earlier revision of this line stated — measured against every agent frontmatter on disk (marketplace EVID-231): **Profile B denies the plural read `forgeplan_claims`** (exploration is Profile A/orchestrator territory, AGENT-AUTHORING-GUIDE §claim-hygiene) **and keeps the singular `forgeplan_claim`/`release`** — a reviewer must claim its artifact (Step 1) and release it in the finally-clause (Step 10). Smith (B-orchestrator) is the mirror image: denies `claim`/`release`, keeps plural `claims` for its pre-flight sweep. Read-only agents (research-analyst, coder-tdd) deny the singular claim because they never own work items.
...
- Skills (e.g. `c4-diagram`, `methodology-check`, `decision`, `supersede`) are invoked by agents but are not themselves agents — they don't appear in the forgeplan-aware agent list (42 of 95 at the time of writing).
...
| Last sync with CLAUDE.md | 2026-09-02 (wave 1: profile labels reconciled with disk, rows 13-14 playbooks + cards) |
