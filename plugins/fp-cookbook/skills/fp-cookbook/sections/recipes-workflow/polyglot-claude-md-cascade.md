...
Organize CLAUDE.md files in a Rust + Go + TypeScript + Python monorepo so that
each language sub-agent receives only its relevant conventions — no cross-language
noise, no redundant context.
...
└── python/CLAUDE.md       ← Python: pytest, ruff, pyproject.toml conventions
```
...
- PRD/RFC: see `.forgeplan/`
```

Per-language CLAUDE.md example (`rust/CLAUDE.md`):
...
**Cargo**: workspace deps in root `Cargo.toml`; crate `Cargo.toml` inherits versions
```

## Пример

For PRD-043 (Sprint R polyglot dispatch), a `coder` sub-agent dispatched
to `rust/src/auth/` reads:
1. root CLAUDE.md → stack context, cross-cutting rules
2. `rust/CLAUDE.md` → thiserror, deny-lints, workspace deps

The sub-agent does NOT load `python/CLAUDE.md` — zero context wasted on
pytest conventions when building Rust auth.
...
| Per-lang file conflicts with root (e.g. root says "use tabs", rust says "use 4 spaces") | Root declares ONLY cross-cutting rules; lang files own formatting |
| Sub-agent dispatched to `rust/` writes Python-style `if x == None:` | Verify `rust/CLAUDE.md` exists and has explicit anti-patterns section |
...
- [Claude Code monorepo docs](https://docs.anthropic.com/en/docs/claude-code/projects#nested-claudemd-files)
- `affected-files-discipline.md` — path-prefix bucketing complement to this pattern
