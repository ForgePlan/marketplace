...
> Долговременная межсессионная память для Claude Code. Обёртка над [Hindsight](https://github.com/vectorize-io/hindsight): 27 MCP инструментов, 3 авто-хука, 8 вспомогательных skills и агент-куратор — Claude помнит контекст между сессиями, проектами и неделями работы.
...
| `/fpl-hsmem:correct-memory` | Исправить неверный факт, не уничтожая след: найти, пометить с причиной, записать верное целиком, пересобрать выводы, проверить, что задача завершилась. Вызывается осознанно — у него есть последствия. |
| `/fpl-hsmem:audit-bank` | Аудит только на чтение: включено ли маскирование секретов, сколько стоит один документ, что упало молча. Прогонять в первый же день нового банка. |
| `/fpl-hsmem:directives` | Правила, которым следует синтез. Стабильно кривой по форме ответ — это проблема правил, а не фактов. |
