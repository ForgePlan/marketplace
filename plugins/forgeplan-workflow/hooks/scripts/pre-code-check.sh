...
# Exit 0 = allow (always), prints warning to stderr if no active PRD.
...
if [ -z "$ACTIVE_PRDS" ] || echo "$ACTIVE_PRDS" | grep -qi "no.*found\|no.*active\|empty\|0 artifacts"; then
  echo '{"warning": "No active PRD found. Consider running forge-cycle or creating a PRD with forgeplan new prd before making code changes."}' >&2
fi
