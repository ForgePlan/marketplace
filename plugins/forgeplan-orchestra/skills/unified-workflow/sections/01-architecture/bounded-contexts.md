...
Key MCP tools: `mcp__orch__query_entities`, `mcp__orch__create_entity`, `mcp__orch__update_entity`, `mcp__orch__search_entities`, `mcp__orch__get_entity`, `mcp__orch__read_messages`, `mcp__orch__manage_field`, `mcp__orch__list_fields`, `mcp__orch__move_entity`.

Field values are written through `update_entity` (or inline in `create_entity`) — there is no `set_fields` tool, and both require field/option UIDs resolved via `list_fields`. See `03-fields/custom-fields.md`.
