...
|---|---|
| Trust `get_relations` for dependencies | Relations and custom-field values are two disjoint stores. A task with four `Blocked by` references returns `[]` from `get_relations`. `add_relation`/`remove_relation` **[agent]** do work and write both sides in one call — but relations still cannot be queried via `fieldFilters`, so `Blocked by` remains the dependency mechanism. |
...
| Pass two fields to `groupBy` | The MCP serializer walks only the top level and skips `isGroup` children. Every group returns `count: 0`. One field only — for Area×Phase, run one grouped query per Area. |
