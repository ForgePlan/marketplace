...
- **НИКОГДА** `mcp__orch__send_message`, пока запись в чат не включена для этого пространства явно; после включения — только в чат самой задачи, без `@`-упоминаний, одно помеченное сообщение на событие
- **НИКОГДА** `mcp__orch__delete_entity` — сироты отчитываются, а не удаляются
