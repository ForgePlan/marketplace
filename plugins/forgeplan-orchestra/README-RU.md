...
>
> **Имена инструментов в плагине написаны без префикса** — `query_entities`, никогда не форма с префиксом. Префикс ненадёжен вдвойне: написание зависит от рантайма (Claude Code ставит два подчёркивания между сервером и инструментом, OMP — одно), а **само имя сервера — это то, что записано в `.mcp.json` проекта**: в одном проекте `orch`, в другом `orchestra-elirum`, и проект может законно держать несколько серверов Orchestra сразу (разные пространства, разные личности и права — сессия человека рядом с ограниченным ботом). Поэтому сервер выбирается на уровне проекта, а не плагина. Есть `.claude/orchestra.json` — бери сервер, закреплённый там; подключён ровно один сервер с сигнатурой Orchestra (`query_entities` + `list_fields` + `get_current_context` под одним префиксом) — используй его; серверов несколько, а конфига нет — спроси, никогда не угадывай по имени. Перед первой записью `get_current_context` выбранного сервера обязан совпасть с закреплённым UID пространства.
>
> **Orchestra к тому же поставляет два *варианта* сервера** с разным набором инструментов: собственный endpoint приложения (без авторизации; есть `navigate_to`/`get_ui_context` и рабочее чтение сообщений) и агентный endpoint за Bearer-токеном (есть `add_relation`/`remove_relation`/`approve_action`/`switch_workspace`/`list_workspaces`, но — на 0.141-beta — нет слоя документов, нет создания сущностей, а чтение сообщений виснет). Полная карта различий — в `skills/orchestra-task-cycle/references/failure-modes.md`.
