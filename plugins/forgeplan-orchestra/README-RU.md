...
**Не привязан к трекеру.** Стадии называют *операции* — показать, что можно начать; взять; отметить
шаг; закрыть — а не имена инструментов конкретного продукта. Чем именно является каждая операция,
говорит проект, а не скилл: сначала читается `docs/agents/issue-tracker.md`, если его нет — один раз
спрашивается человек, и только потом трекер определяется по сигнатуре голых имён инструментов. Один
и тот же цикл ведёт Orchestra здесь и Jira, Linear или GitHub Issues в другом проекте.
...
Прогрессивное раскрытие — `SKILL.md` несёт стадии, глубина лежит рядом:

| Файл | Что внутри |
|---|---|
| `references/project-fields.md` | поля этого проекта — конкретное воплощение таблицы ролей |
| `references/query-recipes.md` | два вопроса доски, ради которых эта модель полей и существует |
| `examples/` | полный проход семи стадий, вариант `Blocked`, заведение задачи |
| `assets/` | скелеты описания задачи и итогового отчёта |

Справочники и примеры написаны под Orchestra, потому что этот проект работает на ней — семь стадий
не написаны ни под кого. Это **наша методология**: знание о платформе она предполагает, а не
пересказывает.

### `orchestra-mcp` — справочник по платформе

Что инструменты Orchestra MCP делают, чего они молча не делают, и какой сервер с какой личностью ты
держишь. Методологии ноль: он никогда не говорит, какую задачу брать и когда она готова.
Подхватывается на диагностических вопросах — «почему запрос вернул пусто», «поле не записалось»,
«что значит `failedFields`», «зависло удаление», "why did this return empty".

| Файл | Что внутри |
|---|---|
| `references/failure-modes.md` | как Orchestra отказывает **молча** — читать до отладки |
| `references/query-cookbook.md` | фильтры, обходы, экономия, сверка числа перед отчётом |
| `references/fields.md` | формы значений на чтении и записи, ловушка двух контейнеров, типы, которые не создавать |
| `references/entities.md` | создание, обновление, чеклисты, сообщения, удаление |
| `references/rendering.md` | почему записанный markdown никогда не читается обратно тем же |
...
Измерено на работающем сервере, а не выведено из документации — последний раз против сборки
`0.141.0-beta.20260906211602`, по одному вызову MCP за раз. Один рецепт оказался неверным именно
потому, что его перепрогнали, а не поверили ему. Поскольку в нём нет ничего от нашей экосистемы, он
публикуется и отдельно — под `npx skills add`.
