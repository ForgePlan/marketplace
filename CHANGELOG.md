...
16 shipped skills and commands were loading with empty metadata. The YAML in their frontmatter did not parse, and the runtime dropped every field in it without saying so — `description`, `allowed-tools`, `disable-model-invocation`. Nine of the sixteen are fpl-skills skills, including `/riper`, `/team`, `/do`, `/setup` and `/spec-author`. Fourteen cases were an unquoted value containing a colon and a space; two were quotes nested inside quotes. From marketplace#244.

- **The repair is quoting only.** Description text is preserved word for word, verified by parsing the repaired files back and comparing against the originals. Fields that had been silently absent returned: six skills regained `disable-model-invocation`, four regained `allowed-tools` — meaning until now those tool restrictions were not in force at all.
- **A gate for the class, not the instance.** The new `frontmatter-check` reddened on exactly those 16 files before the repair, and its verdict was cross-checked against a real YAML parser on every shipped file carrying frontmatter — full agreement, no false positives and no misses.
- **Eight plugin bumps plus the catalog, because otherwise the fix reaches nobody.** The plugin cache lives at a path with the version in its name, so with no bump `/plugin marketplace update` has nothing to update. Those bumps also carry the already-merged marketplace#243 to users: Claude Code 2.1.257 began rejecting a `plugin.json` that declares hooks under the unrecognised `components.hooks` key, and a rejected manifest takes the whole plugin down with it — no skills, no agents, no hooks. The field was descriptive only; the real hooks come from `hooks/hooks.json` autodiscovery, present in all six affected plugins.
- **Two gates were passing locally and never running in CI.** `omp-catalog-check` and `interop-skills-check` had been added to `validate-all-plugins.sh` and not to the workflow — the worst direction for that drift, because the author sees ALL PASSED and ships a tree nothing checked. Both are wired now, and `gate-parity-check` asserts the two lists agree, plus that every gate present in `scripts/ci/` is wired into both, so a newly written gate cannot sit unwired in either.
- **`auto-add-to-project` moved to `pull_request_target`.** Secrets are not handed to forks, which is why the first external PR got a red check through no fault of its own. The workflow has no checkout step, so there is nothing to run — and the invariant is now written into the file itself, together with the warning that no gate enforces it: the `WF-PRT-CHECKOUT` rule matches only a narrow form, is switched off by a single `#` inside a step, and does not see a PR branch pulled through `run:` at all.

Bumped: agents-canvas 1.0.0 → 1.1.0, dev-toolkit 1.6.3 → 1.7.0, forgeplan-brownfield-pack 1.5.1 → 1.6.0, forgeplan-orchestra 1.7.0 → 1.8.0, forgeplan-workflow 1.14.0 → 1.15.0, fpl-skills 1.57.0 → 1.58.0, laws-of-ux 1.4.1 → 1.5.0, slopocop 1.0.0 → 1.1.0. Closes marketplace#244 and marketplace#247.

Also in this release: `docs/PROCESS-GAP-MAP-RU.md`, a diagnostic pass over the pipeline dated 2026-08-29 — 13 gaps, each filed as an issue under tracker marketplace#240, every number derived by commands run against disk. It records that NFR has no validator rule at all (29 of 79 PRDs carry the section), that `/forge-cycle` never calls `phase_advance`, and that the product side is absent — 0 product plugins of 23. Corrected here: 95 agents rather than 97, because two files inside `agents/` are a README and a SCAFFOLDING doc; no conflict with the published 94/41, only a blind spot in the counter, which does not descend into `agents/<name>/` and so never sees the `discover` agent.

## [1.138.0] - 2026-08-29

### fpl-skills v1.57.0 — a cross-runtime wiring tool instead of a copy-paste recipe

The portability section documented three `ln -s` commands. That is a recipe, not a tool: it leaves the detection, the ordering and the safety judgement to whoever is reading, every time, in every project. From marketplace#217.

`cross-runtime.mjs` detects which runtimes are actually installed (env vars, config directories, binaries on `PATH`), checks only the surfaces each one reads, reports what it can and cannot see, and with `--fix` creates the missing links. `--all` covers undetected runtimes, `--json` is machine-readable, `--strict` exits 1 for CI. Zero dependencies, Node builtins only.

- **The failure it removes is silence.** A runtime that cannot find a skill does not report a wrong path; it behaves as though the skill does not exist. Codex reads `.agents/skills` and nothing else, OpenCode reads `.opencode/commands` rather than `.claude/commands`, and neither says so.
- **Deliberately conservative about writing.** It creates relative symlinks inside the project, so they survive a clone and never leak a home path. It refuses to delete a real directory sitting where a link belongs — that directory may hold edits which were never in the source — and reports it as BLOCKED. It refuses to write MCP config, which can be user-scoped or shared with other projects, and prints the command instead.
- **The first gap count was wrong**, which is worth recording: it counted surface entries, so `.agents/skills` showed as three gaps because Codex, OMP and agents.md clients all want it. One link closes all three. It now counts distinct paths.

`docs/CROSS-CLI.md` gained the consumer side in the same release — what a project installing these plugins does so its agents can see them, which the file had never covered — plus a traps table, because all four defects found by running this repo in a second runtime share one shape: a runtime difference that presents as a fault in the thing being used. The tool-name prefix reads as "the MCP server is not connected" (it is connected); the catalog name reads as "missing or invalid field name" (the field is there); the denylist produces nothing at all (every denial lapsed); a missing alias reads as "no such skill" (the skill exists). None of them raises an error, so the reader reaches a wrong conclusion instead of a stack trace.

## [1.137.0] - 2026-08-29

### forgeplan-orchestra v1.7.0 — checklist conventions, and a correction to the field claim shipped in v1.6.0

Four conventions were missing, measurably: "Steps (" appeared zero times across the 14 skill sections, "Done when" zero times, nothing described what an item should look like, and nothing said to create the lists in the same turn as the task. From marketplace#214.

- **Naming is one axis — `Steps (<stage>)`.** Type never appears in a checklist name: it is already the Tags field, checklist names are not queryable, and type changes the items rather than the heading.
- **Two lists at creation, gate first**, so a turn cut short keeps the definition of done and loses a to-do rather than the reverse. This is the same gradient the field measurements already showed — `Phase` 27/32 and `Depth` 22/32 are populated because they are written at creation, `Sprint` and `Branch` are 0/32 because they need someone to come back.
- **Item shape is `<imperative action> — <observable proof>`**, with one test: could a competent agent doing this work honestly leave this item unticked? If not, it is decoration. "write the code", "make sure tests pass", "verify it works" all fail it — they get ticked regardless of what happened. This is the expensive failure, not the taxonomy one: unfalsifiable items produced 2633 green tests on a repo whose two HTTP probes were both returning 404.
- **And the correction.** v1.6.0 said "Never modify system fields (Status, Priority, assignee, dates)" and called Status an Orchestra native field. The live server returns Status, Priority and Tags as `type=custom`, `isSystem=false` — nine custom fields on that workspace, not six. Only Project, Parent, Assignee, Members, Due date, Owner and the read-only counters are system. Fixed as marketplace#210 argues rather than by swapping the list: another workspace may genuinely have them as system fields, so the rule is now to resolve `isSystem` from `list_fields` at runtime and never act on a belief about which side a field falls on. Same class as the UID mistake in marketplace#208 — hardcoding something that is per-workspace.

Shipped in the same release, carrying no version of its own: the 34 skills that were unreachable outside Claude Code are now published. AGENTS.md stated unconditionally that plugins publish skills in both `skills/` and `.agents/skills/` — Codex reads only the second, OpenCode reads it alongside its own — while on disk five plugins had no `.agents/skills` at all (agents-canvas, forgeplan-map-pack, slopocop, slopocop-code, slopocop-design) and fpl-skills carried 22 aliases for 41 skills. The missing aliases were created in the same relative form the existing ones use, with a fixer and a read-only gate that treats a dangling symlink as worse than a missing one — it reads as present to a human and as absent to the loader — and that refuses to replace a real directory where a symlink belongs, since that directory may hold edits which never reached `skills/`.

## [1.136.0] - 2026-08-29

### fpf v1.5.0 + agents-sparc v1.4.2 — ship the `/fpf-lookup` that was already promised, link the routing map that was never linked

Two documentation-integrity defects that both send a reader somewhere that does not answer them. From marketplace#215 and marketplace#216.

- **`/fpf-lookup` was documented ten times across six files and did not exist** — three places in METHODOLOGIES, three in its RU twin, the PLAYBOOK use-case row and its RU twin, and UPSTREAM-METHODOLOGIES in both languages. The PLAYBOOK row is the one that bites: it answers a question a user is actively asking with a command that returns nothing. Built rather than removed, because the behaviour already existed — `/fpf` carries a LOOKUP mode with the same contract, and only the standalone entry point was missing. Deleting ten references would have withdrawn a promise to a capability that is real. The new command is a thin router over the same knowledge base, with the routing discipline stated explicitly: open the index, read the narrowest file, do not load 224 sections to define one term.
- **METHODOLOGIES lists 20 methodologies; `routing-map.md` holds 37** (that 37 was itself a miscount — six of the counted headings are not methodology cards; corrected to 33 in 1.140.0, marketplace#229), each with a dispatch chain and an evidence bar — and METHODOLOGIES never linked it, while citing "smith Row 4" and "smith Row 14" by number, which assumes the reader can find the rows. The cost is recorded in the issue: a six-reader study followed the documented trail and not one reader reached `routing-map.md`, then concluded that seven methodologies have no stages, artifacts or gates. Each has a card; five have dedicated section playbooks. The conclusion was wrong because the path does not lead there. Fixed by linking it in See also and by saying which document is canonical — `routing-map.md` is what the router executes, METHODOLOGIES is a curated summary of the 20 rows that have a standalone command — in the EN and RU files both, because half-fixing this is the failure mode the issue names.
- **Dropped "ALL" from the sparc-orchestrator phase-agent row.** It read "ALL reused", which says the plugin ships only an orchestrator, and METHODOLOGIES:126 then contradicts it with "5 agents (orchestrator + 4 phase specialists)". specification, pseudocode, architecture and refinement ship inside agents-sparc. The column header already names each agent's plugin, so the word was doing no work except being wrong.

## [1.135.0] - 2026-08-29

### agents-pro v1.16.0 + fpl-skills v1.56.0 + forgeplan-map-pack v0.20.0 + agents-sparc v1.4.1 — the activate invariant stated in the body, because the denylist exists in one runtime only

`disallowedTools` is a Claude Code frontmatter key. Verified against omp v18.0.8: the string appears zero times in the binary. The keys its agent parser accepts are `name`, `description`, `tools`, `spawns`, `model`, `output`, `thinkingLevel`, `blocking`, `autoloadSkills`, `readSummarize`, `prewalk`, `advisor` — no denylist key, no MCP scope field. This is not a misconfiguration on our side; the surface does not exist. From marketplace#218.

- **42 agents carry a denylist and every one denies `forgeplan_activate`.** In Claude Code that is a wall. Elsewhere it is nothing, and no error is raised. The quiet failure is a Profile B reviewer activating the artifact it just reviewed: generator ≠ verifier collapses and nothing reports it.
- **Nine of the 42 stated the invariant nowhere in their body** and relied on the denylist alone. Those nine now carry it as a HARD RULE, in the section every one of them already had. Six of the nine are in forgeplan-map-pack, the newest plugin: the convention exists and new plugins skip it.
- **AGENT-AUTHORING-GUIDE gains a section on the runtime split, and rule 5 is qualified.** It previously read "HARD RULES are first-class — surface invariants that the denylist cannot enforce", which implies everything else *is* enforced; in a runtime without the key, that set is empty. The section also names the wrong form explicitly: telling a model "the denylist blocks this, so you cannot call it" is worse than silence, because where the denylist is absent it is a false statement about the agent's own constraints.

Two measurement corrections worth recording. The issue undercounts — it says 77 agents and 33 denylists, while disk says 94 (what `catalog-check` asserts) and 42, so the hole is about 27% wider than filed; posted to the issue. And the first measurement taken here was also wrong: a regex for "never call activate" missed "**Never** call `forgeplan_activate`" because markdown emphasis sits between the words, so 27 agents looked silent when only 9 were. Stripping emphasis before matching gave the real number and avoided 18 pointless edits.

## [1.134.0] - 2026-08-29

### fpl-skills v1.55.0 + forgeplan-orchestra v1.6.1 + cc-best v1.1.2 — MCP tool names written bare, so an agent stops reporting a live server as missing

The `mcp__server__tool` form is Claude Code's spelling, not a universal one. OMP collapses the separator to a single underscore, so an agent following our instructions there searches its tool list for a name that does not exist, finds nothing, and concludes the MCP server is not connected. The failure is silent and looks exactly like a genuine outage. From marketplace#212.

Bare names — `query_entities`, `list_fields`, `update_entity` — are correct in both runtimes: the server is established by context. 72 occurrences dropped across 15 files in three plugins, after verifying first that this is safe: zero occurrences of `mcp__orch__` appear in any agent frontmatter or any `hooks.json` across the marketplace, so the caution raised while planning — that stripping the prefix might break a denylist — does not apply to the Orchestra tools.

- **`create_task` does not exist.** ISSUE-TRACKER-TEMPLATE.md instructed agents to call `mcp__orch__create_task`; the live server exposes 33 tools and that is not one of them. Corrected to `create_entity`. Same class as the `set_fields` bug in marketplace#208 — a documented call to a tool that was never there.
- **The detection rule was the worst instance.** `/briefing` decided whether a tracker was available by matching the prefix, which is precisely the check that returns false for a connected server in OMP. It now matches bare tool names, for every tracker in the table, and says why.
- **Scoped deliberately.** 554 prose occurrences of `mcp__forgeplan__` remain marketplace-wide and are not touched here: 35 of them are documentation about the denylist mechanism, where the full string is the subject rather than an instruction to call it. Stripping those blindly would make the documentation wrong. That pass needs per-occurrence judgement and its own change.

Also unified the Orchestra server name — the marketplace carried both `mcp__orch__` and `mcp__orchestra__` for one server, so a detection rule matching one silently missed the other — and added the naming convention to AGENT-AUTHORING-GUIDE so new files do not reintroduce it.

## [1.133.0] - 2026-08-29

### cc-best v1.1.1 — sign your own work, and read CONTRIBUTING before touching the catalog

Two conventions agents were not being told, both surfaced by the OMP work. From marketplace#213.

- **Sign your own work.** The commit trailer was hardcoded to a specific model in three places, and the three had already drifted: AGENTS.md said "Claude", CLAUDE.md named a model that had since been superseded, and cc-best shipped that same stale name to users as an example to copy. A hardcoded model name in a document is wrong the moment a newer one ships, and an agent that copies it signs a name that did not do the work — in a log nobody rewrites. All three are now placeholders for the agent's resolved identity, with the convention stated once in AGENTS.md: name, version, vendor where one exists, across commit trailers, forgeplan claim/EVID/NOTE metadata, and sub-agent output. Attribution is what makes generator ≠ verifier checkable: two entries signed identically are indistinguishable from one entity reviewing itself. An unresolvable version is written as unresolved rather than guessed.
- **Read CONTRIBUTING.** AGENTS.md never linked it, so the procedural rules that the validation script only enforces after the fact were invisible to an agent reading AGENTS.md as its entry point. Added the four that get skipped, each with what it costs — catalog `metadata.version` as the cache-invalidation key, the OMP catalog regeneration, lowercase-kebab plugin names, and the two version fields that drift independently.

The `catalog-check` gate caught the CLAUDE.md version table on the first run, which is the drift it exists for.

Shipped in the same release, carrying no version of its own: `.omp-plugin/marketplace.json`. OMP validates a catalog name against `/^[a-z0-9]([a-z0-9.-]*[a-z0-9])?$/` with no case-insensitive flag, so "ForgePlan-marketplace" fails on two capital letters and the whole catalog is rejected — with an error reading "Missing or invalid field name", which looks like a malformed file rather than a naming rule. Reproduced against omp v18.0.8, then proven by control: an identical copy with only the name lowercased is accepted. Renaming the existing catalog would break every installed plugin ID carrying the marketplace suffix, so a second catalog ships instead — OMP reads `.omp-plugin/marketplace.json` first and falls back to `.claude-plugin/`, and Claude Code never looks at the new path. The file is committed rather than built in CI because OMP reads a clone of this repo, so a CI-only artifact would never reach a user's disk; the new gate asserts the two stay in sync and never writes, and the generator and the gate share one transform function so the fixer and the assertion cannot disagree about what "in sync" means.
