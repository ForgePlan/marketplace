...
- Чтобы **понять поверхность методологий** — `/smith-routing` показывает 12 контекстов + 29 карточек методологий без обязательства привязки к задаче.
...
- `plugins/fpl-skills/skills/smith/routing-map.md` — таблица 12 контекстов + 29 карточек + индекс агентов.
