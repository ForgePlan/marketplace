...
Smith живёт в `plugins/agents-pro/agents/smith.md` (сам агент) плюс `plugins/fpl-skills/skills/smith/` (мозг с 14 контекстами) и доступен из любого CLI, поддерживающего стандарт AGENTS.md.
...
# Образовательный обход 14 строк маршрутизации
/smith-routing
```
...
## 14 контекстов, которые smith маршрутизирует
...
| 13 | TDD-фича (тесты заморожены до кода) | Сначала тесты, заморозка оракула — Enforced-TDD (RFC-012, ADR-010 #1, hook-gate=Yes) |
| 14 | Дизайн-система → код (Pencil/Figma → Storybook → нативный код фреймворка) | Перенос дизайна в компоненты — CANVAS (RFC-021, ADR-010 #5, hook-gate=Yes) |

Строки 13 (TDD) и 14 (CANVAS) — инстансы sub-cycle-контракта AD/AID-PDLC (ADR-010), обе `hook-gate=Yes`, каждая диспатчится через своего мастера (`tdd-orchestrator`, `canvas-coordinator`).
...
2. **Route** — сравнивает intake с 14 строками `routing-map.md`. При неоднозначности диспатчит FPF ADI (`forgeplan_reason`), чтобы предъявить ≥3 кандидатских строк + рекомендацию.
...
Таблица 14 контекстов **CLI-agnostic** — она называет методологии и роли Profile-A/B/C/D, а не Claude-специфичные примитивы. Каждый CLI мапит названия профилей на свою модель диспатча.
