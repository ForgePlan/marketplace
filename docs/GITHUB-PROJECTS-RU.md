...
| Источник | На доску | Поле Kind | Дефолтный Status |
|---|---|---|---|
...
PR автоматически labelятся по conventional-commit префиксу title (`fix(...)`, `feat(...)`, `docs(...)`, `chore(...)`, `audit(...)`, `refactor(...)`). Auto-add workflow от labels не зависит, но `/gh-project add-pr` читает префикс title чтобы установить поле Kind.
...
|---|---|---|---|
| **Status** | single-select (built-in) | `Backlog`, `Ready`, `In progress`, `In review`, `Done` (note: `Cancelled` not in default GitHub schema; add via UI or remap `deprecated`→`Done`) | да |
| **Kind** | single-select | `PRD`, `RFC`, `ADR`, `Feature`, `Bug`, `Docs`, `Chore` | да |
...
- **Kind** разделяет архитектурные артефакты (PRD/RFC/ADR) от исполнительной работы (Feature/Bug/Docs/Chore).
...
```bash
# Single-select пример (Kind field с несколькими опциями)
...
# → добавляет на доску с Kind=PRD, Forgeplan-ID=PRD-024, Status=Backlog

# Реализация
git checkout -b feat/magic-link-auth
# ... работа ...
gh pr create --title "feat(auth): magic-link admin login (PRD-024)"
# → workflow auto-добавляет PR на доску, Kind=Feature, Status=In Review
...
# → workflow auto-добавляет PR на доску, Kind=Docs, Status=In Review
