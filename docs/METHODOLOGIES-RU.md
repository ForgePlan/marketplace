...
**Статус**: поставляется в плагине `agents-canvas` (v1.0.0) как команда-оркестратор `/canvas` — **пятый инстанс** контракта подцикла AD/AID-PDLC (ADR-010 / RFC-021; стек-агностичный нативный single-framework дефолт — по RFC-022 / ADR-015), рядом с `/tdd` (RFC-012), `/bmad` (RFC-013), `/sparc` (RFC-016) и `/riper` (RFC-018).
...
**Опциональный, вне-дефолтный путь (будущий ADR-016)**: если проект явно запрашивает мульти-фреймворк порт, добавляется шестая фаза — **Spread** → `canvas-porter-framework` (обёртки React / Vue / Svelte / Angular / Solid — параллельный fan-out, один агент на пакет, `blockedBy` код-гейта) плюс отдельный Gate Parity. CANVAS сохраняет своё имя уже на дефолтных пяти фазах — это собственное имя пайплайна, а не описание его выходного формата; опциональная шестая буква — Spread — не является условием для имени, а лишь необязательным расширением; по умолчанию эта фаза не запускается.
...
В таблице **20 методологий — те, у которых есть отдельная команда или плагин**. Это
выжимка, а не каталог. Каталог —
[`smith/routing-map.md`](../plugins/fpl-skills/skills/smith/routing-map.md), в нём **33
карточки** с цепочками диспатча и планками доказательств; оставшиеся 13 — архитектурные
линзы и маппинги для импорта, у которых своей команды нет.

> Раньше здесь стояло 37 — это была ошибка счёта, а не дрейф: `grep -c '^### '` по
> `routing-map.md` даёт 37, но шесть из этих заголовков относятся к секции про параллельный
> диспатч и карточками методологий не являются. Считать надо саму секцию карточек:
> `awk '/^## Methodology cards/{f=1;next} /^## /{if(f)exit} f&&/^### /{c++} END{print c+0}'`.
> Арифметика «оставшиеся N» наследует от этого числа, поэтому сдвинулась тоже (marketplace#229).

При расхождении прав `routing-map.md` — его читает роутер.
...
- [`smith/routing-map.md`](../plugins/fpl-skills/skills/smith/routing-map.md) — **матрица роутинга, которую smith читает на самом деле**: 14 контекстов и 33 карточки методологий, у каждой цепочка диспатча и планка доказательств. Таблица выше — курируемая выжимка из неё; исполняет роутер именно этот файл. Начинай оттуда, если методологии нет в списке ниже или нужна не сама методология, а последовательность вызовов.
