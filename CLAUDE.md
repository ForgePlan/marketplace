...
## Plugin versions (catalog v1.135.0)
...
| **agents-github** | 1.1.0 | — |
| **agents-sparc** | **1.4.1** (denylists are Claude Code-only — `disallowedTools` appears zero times in the OMP binary, so the activate invariant now lives in the agent bodies where it travels (marketplace#218)) | SPARC instance #3 (RFC-016) — sparc-orchestrator → B-orchestrator + /sparc skill |
