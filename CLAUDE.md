...
**Agents**: 41 of 94 forgeplan-aware (PRD-026 B2 paradigm — `disallowedTools` denylist; + the AD/AID-PDLC sub-cycle masters tdd-orchestrator / bmad-orchestrator, sparc-orchestrator finished to B-orchestrator, and the C4 verifier tdd-test-validator. **`memory: project` REJECTED Sprint R** — Hindsight covers use case.)
...
| **fpl-hsmem** | 2.1.0 |
| **forgeplan-workflow** | **1.15.0** (манифест больше не отбраковывается на Claude Code 2.1.257 — убран `components.hooks`, marketplace#242. Sprint Z7: Step 4.5 FPF ADI mandatory for Standard+; Sprint Z6: Step 6.5 BMAD adversarial review mandatory for Standard+) |
...
| **agents-domain** | 1.1.0 | — |
| **agents-pro** | **1.16.0** (denylists are Claude Code-only — `disallowedTools` appears zero times in the OMP binary, so the activate invariant now lives in the agent bodies where it travels (marketplace#218)) | EPIC-003 guardian Step 5 rows + EPIC-002 smith B-orchestrator |
| **agents-github** | 1.1.0 | — |
