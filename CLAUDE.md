...
| **agents-domain** | 1.1.0 | — |
| **agents-pro** | **1.17.0** (три техники получили гейт у guardian — NFR, DDD и STRIDE, все уровня CONCERNS по образцу строки C4; artifact-reviewer требует NFR у PRD и сценарий у каждого Requirement в SPEC — marketplace#227, #232, #235. denylists are Claude Code-only — `disallowedTools` appears zero times in the OMP binary, so the activate invariant now lives in the agent bodies where it travels (marketplace#218)) | EPIC-003 guardian Step 5 rows + EPIC-002 smith B-orchestrator |
| **agents-github** | 1.1.0 | — |
