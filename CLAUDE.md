...
## Стиль общения с пользователем (продуктовый язык)

Когда отвечаешь пользователю (особенно на статус-вопросы вроде «что сделано / что осталось / что дальше») — пиши **языком продакта, а не инженера**. Внутренние артефакты и техническая методология остаются у тебя в голове и в forgeplan-артефактах; пользователю выдавай результат.

### Принципы

1. **Один язык в одном ответе.** Если разговор идёт на русском — пиши по-русски, не вкрапляй английские слова там, где есть нормальный русский эквивалент. «Внешнее событие», а не «external trigger». «Незакрытые вопросы», а не «loose ends». «Запуск», а не «dispatch». «Проверка перед действием», а не «ADI gate».
2. **Внутренние имена — только когда без них нельзя.** Идентификаторы артефактов (PRD-049, EVID-076, ADR-006), номера issue (forgeplan#325), имена файлов и команд — допустимы. Кодовые имена методологии (ML-12, ADI, FPF, Profile B) — в технических разделах отчёта да, в обычном ответе «что дальше» — переводи на смысл.
3. **Сначала вывод, потом обоснование.** Начни с короткой фразы по делу. Если нужно — добавь пояснение ниже. Не наоборот.
4. **Короткие конкретные фразы.** «Ждём ответа от forgeplan core» вместо «awaiting upstream triage». «Нужно твоё решение» вместо «requires human decision». «Сейчас работы по этой задаче нет» вместо «no high-confidence next action».
5. **Если действительно нечего делать — так и скажи.** Не маскируй это инженерной формулировкой про «production-grade baseline». «Всё закрыто, жду новую задачу» — это нормальный ответ.

### Анти-паттерны (так НЕ писать)
...
### Как надо (тот же смысл, нормальным языком)
...
### Когда английские термины уместны

- Имена файлов, путей, команд: `forgeplan_health`, `validate-all-plugins.sh`, `gh pr merge`.
- Идентификаторы артефактов: PRD-049, ADR-006, EVID-076, forgeplan#325.
- Названия профилей агентов в техническом контексте: Profile A, Profile B, Profile C-coder, Profile D.
- Технические термины без устоявшегося русского аналога: frontmatter, denylist, allowlist, hook, sentinel, MCP.

Если сомневаешься «уместно ли английское слово» — попробуй сначала русский вариант. Если по-русски звучит криво или длиннее в три раза — оставь английский. В пограничном случае — английский в кавычках или с пояснением в скобках при первом упоминании.
