...
**Catalog version**: 1.184.0
**Plugins**: 23 (14 workflow + 8 agent packs + 1 memory plugin fpl-hsmem) — adds the AD/AID-PDLC sub-cycle instance packs agents-tdd (#1) + agents-bmad (#2) + agents-canvas (#5 — CANVAS design-system→code, RFC-021, hook-gate=Yes) alongside agents-sparc (#3, finished to B-orchestrator) + smith master-orchestrator (EPIC-002, Profile B-orchestrator)
**Agents**: 44 of 97 forgeplan-aware (the gate-enforced definition: carries a disallowedTools denylist, PRD-026 B2) + 3 allowlist-backed Profile B reviewers outside that definition (injection-analyst, pii-detector, performance-engineer — rows 8-9 fix, 2026-09-05: full forgeplan EVID surface, constraints enforced by omission) (PRD-026 B2 paradigm — `disallowedTools` denylist; + the AD/AID-PDLC sub-cycle masters tdd-orchestrator / bmad-orchestrator, sparc-orchestrator finished to B-orchestrator, and the C4 verifier tdd-test-validator. **`memory: project` REJECTED Sprint R** — Hindsight covers use case.)
...
- **Plugins + versions** → `.claude-plugin/marketplace.json` (23: 14 workflow + 8 agent packs + 1 memory)
...
## Three registers called "phase" (ADR-022)

Three different things in this project are named "phase". Conflating them left a normative RFC carrying a stage count in its title that its own table contradicted, unnoticed for months.

| Register | Values | Carrier |
|---|---|---|
| **pipeline stage** | `brief` `shape` `decompose` `design` `estimate` `gate` `build` `audit` `evidence` `activate` `wrap` | `plugins/fpl-skills/templates/project-agent-matrix.yaml` — the count is derived from its keys, never restated by hand |
| **session phase** | `idle` `routing` `shaping` `coding` `evidence` `pr` | `forgeplan_session` / `forgeplan_guard` |
| **artifact lifecycle marker** | `shape` `validate` `adi` `code` `test` `audit` `evidence` `done` | `.forgeplan/state/<id>.yaml`, read via `forgeplan_phase` |

Rules that hold across the repo:

- Do not write bare "phase" for the pipeline unit — say **stage** (INV-4). Reference a stage by **name**, never by position number (INV-5).
- The lifecycle marker is **advisory**: a resume hint, never a gate condition (INV-2). Gates read `status`, evidence and R_eff.
- Do not extend the marker enumeration (INV-1), and do not bulk-advance markers to make `forgeplan_health` green (INV-3) — the aggregate is a real measurement of how the pipeline ran.
- `scripts/ci/phase-canon-check.js` enforces the stage count on every document in this repository. RFC-002 lives above the git root and stays a human responsibility.
...
**Sign with what you actually are.** The trailer is a placeholder, not a string to copy —
fill it with your resolved model identity (name, version, vendor context). This line
previously hardcoded a specific model, which was already superseded by the time anyone
read it, while `AGENTS.md` hardcoded a different one. Two documents disagreeing about who
signs the commits is exactly the drift a hardcoded example produces.

If you cannot resolve your own version, write it unresolved rather than guessing — a
trailer naming a model that did not do the work is a false record in a log nobody rewrites.
Full convention: [`AGENTS.md` → Sign your own work](AGENTS.md#sign-your-own-work--with-what-you-actually-are).
...
- For **learning the methodology surface** — `/smith-routing` walks the 14 routing rows + 33 methodology cards without committing to a task.
...
| `/smith-routing` | Educational walkthrough of 14 routing rows + 33 methodology cards |
...
### G9 — REFRESH real-reconciliation vs rubber-stamp (ADR-020, later addition)

**The gap**: ADR-020 materializes REFRESH as the Deep/Critical cycle-close node whose value depends on the body being the *actual output* of running reconciliation (`forgeplan_drift` + `forgeplan_stale` + `/forge-heal` + a link audit), not a self-reported "all good." An agent can write `## Ready verdict: READY` with fabricated-looking reconciliation prose that no parser can distinguish from genuine pasted output.

**What IS machine-closed (the structural half)**: the guardian Step 5 «REFRESH structural completeness» check BLOCKS-to-CONCERNS a `refresh` whose body is missing `## Synced` / `## Gaps closed` / `## Links updated` / `## Ready verdict`, or whose readiness assertion carries no reconciliation-output block at all. Section presence + verdict presence are structurally decidable.

**Why the rest is deferred (accept-by-design)**: distinguishing *genuine pasted* drift/stale/heal output from *fabricated* output is semantic, not structural — the same brittleness class as G6 (BMAD content-spoof) and G7 (ADI hypothesis-spoof). A parser strict enough to catch a fabricated reconciliation would false-positive on a legitimate short one; the reliable detector is LLM-grade, not a hook.

**Social discipline**: the REFRESH author's identity is on the artifact; a pattern of rubber-stamp REFRESHes is visible across cycles. The reviewer chain (and the ground-truth-verification discipline — `completed`/READY means *you re-ran the reconciliation yourself*) is the enforcement layer. Same rule shape as G5/G6/G7: structural where possible, social where not.
...
| `plugins/forgeplan-brownfield-pack/docs/discover/README.md` | Dispatch examples, modes, skill orchestration, Anomaly #14 handling |
...
## Plugin versions (catalog v1.184.0)

### Workflow plugins

| Plugin | Version |
|--------|:-------:|
| **fpl-skills** | **1.74.0** (третий экземпляр одного класса дефекта закрыт: денилисты только для Claude Code, имена инструментов без префикса — и ровно так же `opus`/`sonnet`/`haiku` это имена Claude Code, а требование агента это ЯРУС. Гайд получил подраздел о том, что значение во фронтматтере это привязка, а требование живёт в теле: ярус берётся из девяти ярусов задачи §6.2 и здесь НЕ пересказывается, модель за ярусом выбирается один раз на уровне машины, один `model:` покрывает всего агента и потому ярус задаёт самая трудная стадия, при реальной цене агента режут, а не понижают, верификатор никогда не ниже генератора. Гейта пока нет сознательно — секция есть у новых агентов и отсутствует у большинства существующих. v1.71.0 (карта роутинга получила именованное НЕ-правило: работа с задачей на доске — это конверт, а не пятнадцатая строка. Каждая из 14 строк отвечает на вопрос «как это строим», и smith выбирает ровно одну; «взять задачу с доски» отвечает на другой вопрос — «над какой единицей работы мы сейчас и говорит ли доска про неё правду» — и складывается со строкой, а не конкурирует с ней. Поэтому конверт стал агентом `task-runner`, который на стадии работы диспатчит самого smith: task-runner → smith → строка → её последовательность диспатчей. Четырнадцать строк не изменились; индекс агентов дополнен одной записью. Плюс починен шаблон трекера, который `/setup` пишет с самого начала: он отвечал на два вопроса из семи, поэтому проект, настроенный через `/setup`, оставлял рабочий цикл догадываться об остальных — теперь в нём таблица канонических операций, таблица ролей полей, факты о соединении и личности и постоянные правила; секция A определяет трекер по ГОЛЫМ именам инструментов (её собственный пример писал имя с префиксом вопреки marketplace#212) и при ничьей спрашивает, а не выбирает. v1.70.0: (аудит правки строк 8-9 принят: секции 08/09 переписаны под новые способности + контракт взаимодействия (кто в каком контексте, где человек, где FPF, какие скиллы); perf-baseline.md больше не предзаполняет ось доказательств константой supports и несёт обе оси; LR-4 ловит NotebookEdit, новый LR-9 требует Prompt-defense-преамбулу у forgeplan-aware; кинжал error-detective снят, счёт кинжалов пересчитан. v1.69.0: (строки 8-9 карты роутинга перестали требовать невыполнимого: у трёх ревьюеров (injection-analyst, pii-detector, performance-engineer) аллоулисты не содержали ни одного forgeplan-инструмента, а строки требовали от них EVID — теперь у всех трёх поверхность профиля B, сноска-кинжал и индекс синхронизированы. v1.68.1: сноска-кинжал карты роутинга несёт замеренный нюанс: большинство †-агентов ограничены аллоулистом — жёстче денилиста; метка врёт только там, где аллоулист ей противоречит, и три таких противоречия закрыты (marketplace#236). v1.68.0: (волны 3-4 закрывают ADR-024: шаблон A3-листа (обещанный секцией 11 с первой ревизии) и шаблон perf-базы (p50/p95/p99 + команда воспроизведения — база без неё театр); debt_close в loops: с переиспользованием проверки дельта-спек из /decay-watch; реестр шаблонов честно считает 7; третий экземпляр мислейбла research-analyst закрыт; 25 пустых чекбоксов трёх петлевых секций заменены. /tech-debt и DORA сознательно не строятся — под parseable-триггерами RT-2/RT-3. v1.67.0: (ADR-023 D2+D3: NEW /discover-product — первый продуктовый исполняемый вход маркетплейса: Double Diamond как собственный проход скилла, JTBD и assumption-mapping как саб-моды, допущения ложатся hypothesis-артефактами и двигаются forgeplan_hypothesis_promote (первый исполняемый вызывающий обоих hypothesis-инструментов; создание HYP — только CLI по замеренной необходимости; заглушки interview_packet_* не вызываются никогда — marketplace#265); секция 10 smith починена — research-analyst честно C, персистит оркестратор, artifact-reviewer как C4 для NOTE+PROBLEM, маршрут через новый скилл. v1.66.0: (волна 2 ADR-024: NEW /incident — двухфазная петля (пожар по ICS + blameless post-mortem на существующем шаблоне), бинарная граница с /riper (активная деградация выигрывает пересечение, проверено живьём на роутере); /gate-check получил режим --loop <имя> и новую секцию loops: в порогах (ключ — ИМЯ петли, не глубина; неизвестное имя — громкий FAIL, урок мёртвого дозора EVID-249 G1); зеркало под-ступени ship в /autorun. v1.65.1: (гайд перестал недосчитывать собственных оркестраторов: в бюджетном пассаже стояло 4 при 5 на диске — map-orchestrator выпал из списка; счёт назван «за краем задуманного», отказ ADR-023 от шестого — прецедент. v1.65.0: (пять скиллов получили инструменты forgeplan, которые существовали и не вызывались ничем — каждый прозондирован вживую до подключения: /gate-check — contradictions (честная метка: v1 ловит только дубликаты заголовков) + orphans условно (brownfield-only, в остальных местах всегда пуст — вызов там изготовил бы проверку, которая ничего не проверяет); /wrap — coverage + capture; /decay-watch наконец зовёт одноимённый forgeplan_decay (цена замерена: >2 мин на графе 385); /forge-heal — undo_last строго для деструктивных операций; /progress-dashboard — forgeplan_progress; marketplace#263. v1.64.0: (три дефекта карты роутинга, замеренные по фронтматтеру агентов (EVID-231): политика claim/claims в :351 была изложена наоборот; discover помечен B при теле, объявляющем A; в строке 12 автором post-incident PRD назван агент, которому денилист запрещает создавать что-либо. v1.63.0: (NEW `/estimate` и `/wrap` — две стадии конвейера, которых не было. Обе стояли в матрице как `primary: "inline"` — заглушка со смыслом «оркестратор напишет пару строк»: `forgeplan estimate` существовал, и на пути конвейера его никто не вызывал, а REFRESH из ADR-020 дошёл до гайда по агентам и до карты роутинга, но не до исполнителя — слова `refresh` в `/forge-cycle` не было вовсе. `/estimate` докладывает ПРИЧИНЫ уверенности, а не только число, и ставит рядом снимок R_eff; `/wrap` прогоняет drift + stale + `/forge-heal` + аудит связей и на Deep/Critical записывает их ФАКТИЧЕСКИЙ вывод в REFRESH. Новая заглушка матрицы `skill:<name>` — стадия, которую оркестратор делает сам, а не передаёт агенту (marketplace#233, #234). Тем же релизом хук проверки доказательств перестал блокировать работу, которую не может починить: артефакты он теперь берёт из диапазона base..HEAD, а не из плоского `git log -20` (ссылка в чужом уже влитом коммите блокировала посторонние PR), а ID, которого нет в графе, докладывается как ненайденный, а не как «нет доказательств» с невыполнимым советом — связать EVID с несуществующим артефактом нельзя. Оба поведения закреплены новым набором тестов, проверенным мутацией; `validate-all-plugins.sh` теперь гоняет наборы тестов плагинов, которые до этого гонял только CI. v1.62.0: NEW `/gate-check` — гейт качества между стадиями, которого RFC-002 INV-1 требовал как «единственный механизм перехода от design к build», а на диске не было: переход держался на порядке шагов внутри промпта (marketplace#237). Плюс `templates/quality-gates.yaml` — декларативные пороги по глубинам, must блокирует, should сообщает. v1.61.1: два вердикта, две оси — marketplace#251: `verdict` берёт словарь доказательств (supports/weakens/refutes), `review_verdict` берёт PASS/CONCERNS/BLOCKER. Гайд раньше учил обратному, и 13 тел доказательств честно последовали инструкции. Промах молчаливый и хуже пустого поля: оценщик читает непонятое значение как *supports*, то есть предупреждение считается свидетельством ЗА. v1.60.0: канон трёх регистров «фазы» доведён до исполнителей — ADR-022: `/forge-heal` и `/forge-cleanup` больше не двигают маркеры пачкой ради зелёного health и лишились ложного обоснования «carries no semantic loss» (INV-3); `insight-watchdog.sh` и `/forge-insight` перестали глушить `phase_mismatch` целым видом — benign `shape`/`validate` по-прежнему тихо, а маркер вне этого набора всплывает в обход порога severity, потому что детектор ставит `low` всем 306 (INV-1); шаблон `project-agent-matrix.yaml` объявлен носителем счёта и говорит «стадия», не «фаза»; новый `tests/test-insight-watchdog.sh` — 8 кейсов в обе стороны, проверен мутацией. v1.59.0: rows 13-14 наконец получили playbook и карточки методологий; счётчики методологий 29→33 и агентов 94/41→95/42 сведены к диску; фазовые маркеры подключены к /autorun — marketplace#238, #229, #228. 9 скиллов снова грузятся с метаданными — `/riper`, `/team`, `/do`, `/setup`, `/spec-author`, `/agent-advisor`, `/bootstrap`, `/fpl-init`, `/migrate-from-dev-toolkit`: двоеточие в незакавыченном `description` рушило весь frontmatter, из-за чего `allowed-tools` и `disable-model-invocation` молча не применялись — marketplace#244. NEW `/cross-runtime` — real tooling instead of a copy-paste recipe: detects installed runtimes, reports what each can see, creates the missing links, refuses to delete anything; marketplace#217. v1.56.0: AGENT-AUTHORING-GUIDE: denylists are Claude Code-only, so any invariant that matters must also be a HARD RULE in the body — marketplace#218; v1.55.0 (AGENT-AUTHORING-GUIDE tool-naming convention: write bare tool names in prose — the `mcp__server__tool` prefix is Claude Code-specific, OMP collapses it to one underscore, so a prefixed name makes an agent report a live server as missing; `/briefing` now detects MCP servers by bare tool name, marketplace#212; v1.50.2 marketplace docs CANVAS-completeness sync — AGENT-AUTHORING-GUIDE 12→14 context + canvas-coordinator listed as a Profile B-orchestrator + CANVAS added to all methodology/plugin/onboarding lists; v1.50.1 CANVAS smith routing-map Row 14 + docs/SMITH reframed to native single-framework, RFC-022; v1.50.0 CANVAS smith routing row + 'Parallel dispatch & orchestration discipline' note in routing-map + AGENT-AUTHORING-GUIDE pointer, RFC-021; v1.49.1 scanner patch — /decay-watch + decay-reminder.sh treat NOTE-013 `Kind: finding` as a separate machine track (excluded from human-deferral counts/triggers), RFC-019 R-8; v1.49.0 self-aware integrity brick 2 — /forge-heal skill: Layer 4 heal-or-task, propose-not-silent 3-tier (AUTO/ADI/USER), AUTO fixes 2 reversible kinds only, USER→NOTE-013 task, PRD-074/RFC-020; v1.48.0 brick 1 — /forge-insight + insight-watchdog + accurate session counts + decay-reminder flag fix, PRD-074/RFC-019; v1.46.0 /conformance-vectors SDD enrichment, DEFER-012; v1.45.0 /autorun RIPER Plan→Execute human gate, DEFER-016; v1.44.1 /riper FR-5 pin-hash basis, DEFER-018; AD/AID-PDLC instances #1-#4 smith routing + /methodology-check Step 10; EPIC-002 smith 4-skill cluster) |
| **cc-best** | **1.1.5** (раздел про манифест больше не выдаёт инвентарь `components` за реальную форму ForgePlan — поле убрано из всех 23 манифестов (marketplace#246): оно расходилось с диском при каждом добавленном скилле, и его никто не читал. v1.1.4: доставка правки счёта стадий из #254 — сам контент ушёл без бампа версии и до пользователей не дошёл. v1.1.2: MCP examples write tool names bare; commit-trailer example is a placeholder for the agent's own identity — a hardcoded model name goes stale and gets copied into commits by a model that is not it) |
| **fpl-hsmem** | **3.6.3** (опорное число размера репозитория оказалось НЕ ЗАМЕРЕННЫМ: «~1 674 файла под git» перенесено из `AGENTS.md` того самого репозитория, который этот документ исправляет за два ДРУГИХ непроверенных вывода. Цифра «на диске» рядом была настоящей — воспроизводится с точностью 0.06 %; цифра «под git» — нет: на дату замера там было **2 154**, а 1 674 не встречается ни в одной точке окна (1 926 → 2 102 → 2 117 → 2 154). Исправлено во всех пяти местах, куда успело разойтись. Вывод переживает, якорь — нет: 2 154 в той же полосе, и независимый перезамер снова даёт проигрыш индекса (36 мс без индекса против 39 мс с ним, лучшее из трёх). Это ровно тот промах, о котором документ и предупреждает — правдоподобное число, перенесённое без перепроверки, а затем ставшее границей полосы; нашёл независимый аудит, не автор, и оно уже было отгружено. v3.6.2: поправка к собственной главе, а не тихая переформулировка: 3.6.1 объявил сужение охвата «настоящим лечением» — неверно, оно двигает границу. Четыре замера, а не три (охват × размер раунда) на банке 41 972 памяти / 2 063 526 связей: весь банк при 100 — зависание на неделю и 305 повторов; весь банк при 25 — доходит до конца, но 7 повторов за ~35 минут ради +12 наблюдений; по одному тегу при 25 — 3 задания из 5 с НУЛЁМ повторов, 1 упало, 1 ретраилось; по одному тегу при 100 — единственное задание упало. Обе ручки влияют, ни одна не лечит. Вдобавок подписи строк врали: все чистые узкие прогоны шли при размере раунда 25, а не «по умолчанию» — вывод переживает (строки 2 и 3 держат раунд постоянным и меняют только охват), подпись нет. Вывод, на который числа указывают на самом деле: меньшие банки того же развёртывания (4 863 и 7 011 памятей) консолидируются целиком без всякой подкрутки, поэтому повторное кручение этих ручек — сигнал делить банк, а не крутить дальше. v3.6.1: справочник получил главу о консолидации, которая перестаёт успевать, и ловушку, стоившую здесь недели: пока задание стоит в очереди, каждый следующий запуск возвращает ЕГО ЖЕ и отвечает `200` — одно неспособное завершиться задание блокирует консолидацию навсегда и молча. Тем же релизом восстановлен собственный журнал изменений: 3.1.0…3.6.0 отгружены без записей, шесть релизов существовали только в коммитах слияния. v3.6.0: советы по поиску стали зависеть от размера проекта — по числу файлов ПОД git (`git ls-files`, не `find`), и замеры помечены как наблюдения одного репозитория, а не пороги: на 1 674 файлах индекс ПРОИГРАЛ (29 мс без индекса против 36 мс с ним), а расхожий выигрыш давало не индексирование, а отказ сканировать вендоренный код — он бесплатен и без индекса. v3.5.0: три описания плагинов были обрублены посреди слова срезом `[:6000]` — восстановлены дословно, добавлен гейт формы описаний. v3.4.0: `RETRIEVAL-AND-MEMORY.md` (какой вопрос какому слою и чем проверяется; ADR/RFC/PRD — файлы в репозитории и они ВЫШЕ памяти) + план разграничения банков. v3.3.0: исправлено наше собственное неверное утверждение — `allowed-tools` это ПРЕДОДОБРЕНИЕ, а не ограничение; ограничивает только `disallowed-tools`. v3.2.0: 27 денилистов молча разошлись с реестром — введён единый источник `MEMORY_WRITE_TOOLS` и гейт, выводящий списки из него. v3.1.0: релей объясняет сам себя через `instructions` протокола, ошибки говорят что делать, добавлен `/memory-setup`) |
| **forgeplan-workflow** | **1.23.0** (волна 1 ADR-024: NEW Step 9.7 — под-ступень ship (Standard+): фиксирует отгруженное и регистрирует окно наблюдения parseable date-строкой в NOTE-013; Phase 7.3 поглощён Step 9.5 и удалён — тот же инструмент, тот же вызов, плюс регистровая коллизия «Phase внутри Step» прямо в заголовке; инструмент чейнджлога теперь вызывается ровно в одной секции. v1.20.0: (Step 1 читает forgeplan status + session — устаревшая фаза сессии теперь произносится вслух, а не молча противоречит циклу; marketplace#263. v1.19.0: (последние непроведённые ступени получили шаги: Step 2.5 — brief-intake для сырой идеи, Step 4.4 — goal-planner с forgeplan_decompose на Deep+ (инструмент был назван везде и не вызывался нигде), Step 8a — настоящий диспатч guardian перед активацией вместо обещания в прозе (EVID-231). v1.18.0: (две последние неисполнявшиеся стадии наконец работают: у Step 4.65 появилась вторая половина — вызов `/estimate` на Deep+ (половина про глубину была, про трудозатраты — нет), а новый Step 10 запускает `/wrap` ПОСЛЕ коммита, потому что `forgeplan drift` до коммита меряет рабочее дерево, которого никто, кроме тебя, не видит — marketplace#233, #234. v1.17.0: Step 4.7 и Step 7.4 — два вызова `/gate-check`, pre-build и post-build; до этого гейт-стадия конвейера не исполнялась вовсе, marketplace#237. v1.16.1: /forge-cycle наконец двигает фазу артефакта — 300 из 331 активных висели на ранней фазе, потому что её двигали только скиллы починки, marketplace#228. манифест больше не отбраковывается на Claude Code 2.1.257 — убран `components.hooks`, marketplace#242. Sprint Z7: Step 4.5 FPF ADI mandatory for Standard+; Sprint Z6: Step 6.5 BMAD adversarial review mandatory for Standard+) |
| **forgeplan-orchestra** | **3.4.0** (заголовок свежести в `failure-modes.md` перестал недоговаривать: полный проход был 09-06, точечные перепроверки — 09-08 на более новой сборке, и по их итогам одна строка ПОХОРОНЕНА; теперь две даты названы разными по силе, а непомеченная строка честно держится на 09-06. Плюс правило прямым текстом: перестала воспроизводиться — удаляй, а не смягчай. v3.2.0 (`task-runner` получил секцию `## Model tier` — просит ярус B и объясняет, почему ярус задаёт не самое частое, что агент делает, а самое трудное: четыре гейта, каждый из которых есть отказ быть приятным. `model: sonnet` там назван привязкой Claude Code, а не требованием: на OMP/OpenCode/Codex/Gemini этих имён нет, подставляй модель своего яруса и при промахе промахивайся ВВЕРХ. Экономия — резать агента, а не понижать. v3.1.0 (замеренная находка из потребляющего проекта доехала до справочника: **запись в поля и запись в чат управляются раздельно, и решает членство, а не endpoint**. Бот, только что обновивший поля задачи, получил `7 PERMISSION_DENIED` на публикацию в чат той же задачи; `add_members` с его uid вернул `added: true`, и то же самое сообщение прошло (проверено здесь независимо: бот числится участником в роли editor, сообщение лежит в чате под его uid). То есть отказанный отчёт чинится — но починка УВЕДОМЛЯЕТ всех, кто уже на задаче, поэтому это решение владельца, а не удобство агента. Записано в `failure-modes.md` и в `entities.md`, а стадия 5 в `task-cycle` получила следствие: разрешение в твоём конфиге это не разрешение трекера. Вторая находка той же сессии — будто целые диапазоны задач невидимы для `query_entities`/`search_entities` — ПЕРЕМЕРЕНА здесь на сборке `0.141.0-beta.20260908001639` и ОПРОВЕРГНУТА (проект отдаёт все 13 задач, названная среди них), поэтому сознательно не документирована. v3.0.1 (девятое зеркало живое: репозиторий `ForgePlan/orchestra-mcp` создан, содержимое залито, строка добавлена во все три списка синхронизирующего workflow — флаг `mirrored: false` снят. По дороге закрыта настоящая утечка: оговорка взаимного исключения называла СОСЕДНИЙ скилл, а в установке из одного скилла это ссылка в никуда — теперь она отсылает к «тому рабочему циклу, которым ты пользуешься», а имя соседа добавлено в запрещённые токены зеркала (проверено мутацией живьём: возврат имени роняет гейт с указанием файла и строки). v3.0.0 (ЛОМАЮЩЕЕ — рабочий цикл переименован и его конфиг переехал: скилл теперь `task-cycle`, а не `orchestra-task-cycle`, и он НЕ ПРИВЯЗАН К ТРЕКЕРУ по построению. Стадии называют канонические операции (что можно начать / прочитать одну / взять / отметить шаг / отчитаться / закрыть) и канонические РОЛИ полей (статус / маркер стадии / кто / какая модель / чем заблокировано), а не имена инструментов одного продукта; чем именно является каждая из них, говорит проект через новый `docs/agents/issue-tracker.md` — поэтому тот же цикл ведёт Orchestra здесь и Jira, Linear или GitHub Issues в другом месте. Порядок разрешения: прочитать этот файл, иначе спросить человека ОДИН раз, иначе определить по сигнатуре ГОЛЫХ имён инструментов, иначе при ничьей — остановиться и спросить; угадывать нельзя, потому что запись в чужую систему извинениями не отменяется. `references/field-model.md` переименован в `references/project-fields.md` и переписан как воплощение таблицы ролей именно этого проекта; примеры остаются явно оркестровыми. НОВЫЙ агент `task-runner` (Profile B-orchestrator в границах трекера): ведёт семь гейтов, до первой записи проверяет, какой сервер и чью личность держит, диспатчит `smith` за методологией и агента профиля A/B за артефактами, сам не пишет ни исходники, ни мутации forgeplan — и сознательно НЕ стал 15-й строкой карты роутинга smith: конверт доски складывается со строкой методологии, а не конкурирует с ней. Пин-файл соединения переехал в `docs/agents/orchestra.json` рядом с issue-tracker.md; `.agents/orchestra.json` и `.claude/orchestra.json` ещё читаются и печатают предупреждение в stderr (проверено живьём: устаревший путь даёт выход 0 с предупреждением, канонический побеждает устаревший, stdout под --json остаётся валидным JSON) — marketplace#282. v2.0.0 (ЛОМАЮЩЕЕ — пути файлов переехали: знание о платформе вынесено из рабочего цикла во второй скилл `orchestra-mcp`, свободный от нашей экосистемы — failure-modes, верификатор сервера, скрипт карты полей плюс новые справочники fields/entities/rendering/query-cookbook; ноль forgeplan и ноль наших имён полей, поэтому он самодостаточен и публикуется отдельно под `npx skills add`. `orchestra-task-cycle` сохраняет семь стадий с гейтами, нашу модель полей и путь доказательств, и теперь ссылается на универсальный скилл как на источник истины о поведении инструментов. У обоих описаний явная оговорка взаимного исключения, чтобы они не конкурировали за срабатывание — marketplace#282. v1.12.0: перепроверка живьём против Orchestra 0.141-beta-0906: сборка закрыла недосчёт folder:all, null-фильтры, зависшее чтение сообщений, создание сущностей на агентном endpoint, move_entity, слой документов, порядок чеклистов и плоские project-запросы; failure-modes переписан — оставлено только воспроизводящееся (треды нечитаемы, config write-only, две формы полей) и добавлено два новых (delete_entity виснет глобально без ошибки; демон может отказывать всем инструментам и не восстанавливаться); orch-verify.sh получил код 75 NOT_READY; агентный endpoint задокументирован как личность бота, а не человека — marketplace#282. v1.11.0: резолюция сервера стала детерминированной и нейтральной к рантайму: NEW scripts/orch-verify.sh + пин-файл orchestra.json схемы v2 — канонический путь .agents/, карта имён по рантаймам, tokenEnv; выход 0 = писать можно, 65 = стоп — marketplace#282. v1.10.0: доки перемерены живьём против ОБОИХ вариантов сервера Orchestra: failure-modes получил таблицу раскола endpoint'ов (агентный: add_relation/approve_action есть, но нет create/слоя документов, чтение сообщений виснет, folder:all может уронить целый проект), в примерах восстановлен обязательный checklistUid, subtask-шорткат create_entity переведён на entities[], field-map.sh выучил ORCH_MCP_TOKEN, сервер пинуется через .claude/orchestra.json — marketplace#282. v1.9.0: NEW скилл `orchestra-task-cycle` — рабочий цикл задачи в Orchestra за семь стадий с гейтами (orient → read → claim → work → evidence → report → close), модель полей, рецепты запросов и каталог того, как Orchestra отказывает молча. v1.8.1: манифест больше не отбраковывается на Claude Code 2.1.257 — убран `components.hooks`, marketplace#242. checklist conventions + the field-set correction: `Status`/`Priority`/`Tags` are custom on the measured server, so resolve `isSystem` at runtime rather than hardcoding which side a field falls on — marketplace#214. v1.6.1: tool names written bare — `query_entities`, not `mcp__orch__query_entities`, marketplace#212; `create_task` corrected to `create_entity` (no such tool on the server). v1.6.0: read-before-act on task chats; `Blocked` as a Status never a phase; checklists from acceptance criteria + Done-with-open-items report; `Artifact` backfill from task names with per-row confirmation; pinned workspace UIDs; orphans reported never deleted — marketplace#210, ADR-021. v1.5.0: `/sync` field-write path fixed — the non-existent `set_fields` replaced by `update_entity`, `fields` as `{fieldUid, value}` with UIDs from `list_fields`, `create_entity` on the `entities[]` form; marketplace#208) |
| **forgeplan-brownfield-pack** | 1.7.0 (`causal-linker` и `reproducibility-validator` снова грузятся с метаданными — кавычки внутри кавычек рушили frontmatter, marketplace#244. Sprint V: Discover Agent migrated) |
| **fpf** | **1.7.0** (`/fpf-lookup` shipped — it was promised in 10 places across 6 docs and did not exist, while the LOOKUP behaviour it names was already built inside `/fpf`; marketplace#215) |
| **agentic-rag** | 1.1.1 |
| **fp-cookbook** | 1.2.3 (доставка правки счёта стадий из #254 — контент ушёл без бампа версии) |
| **laws-of-ux** | 1.6.0 (манифест не отбраковывается на 2.1.257 + `/ux-law` снова грузится с метаданными — marketplace#242, #244) |
| **forgeplan-map-pack** | **0.22.0** (denylists are Claude Code-only — `disallowedTools` appears zero times in the OMP binary, so the activate invariant now lives in the agent bodies where it travels (marketplace#218) — 6 of its 8 agents were relying on the denylist alone) |
| **slopocop** | 1.2.0 (манифест не отбраковывается на 2.1.257 + `/slop-humanize` снова грузится с метаданными — marketplace#242, #244. text anti-slop — humanizer-ru + slop-humanizer + /slop-audit + /slop-humanize + slop-cop agent) |
| **slopocop-design** | 1.1.0 (design anti-slop — hallmark + frontend-design + design-taste + /design-audit + /design-redesign + design-cop agent) |
| **slopocop-code** | 1.4.0 (code anti-slop — deterministic 0-100 scanner [JS/TS/Py/Go/Rust/Java/PHP, regex/heuristic dependency-free, per-language idioms] + code-slop skill + /code-audit + /code-deslop + code-slop-cop agent; v1.1.0-1.2.0 dogfood-fixed 3 scanner false positives; v1.3.0 +Java +PHP) |
| **dev-toolkit** | 1.9.0 (манифест не отбраковывается на Claude Code 2.1.257 — убран `components.hooks`, marketplace#242) |

### Agent packs

| Plugin | Version | Last sprint |
|--------|:-------:|---|
| **agents-core** | **1.10.0** (performance-engineer доведён до полного канона B: преамбула, Identity, Step 4.5 (post-change EVID обязан нести Ground-truth verification — иначе guardian его блокирует), Findings-требование с честным нулём; README помечает B и forgeplan-поверхность. v1.7.0: (performance-engineer получил forgeplan-поверхность профиля B и привязку к templates/perf-baseline.md — базовый EVID с p50/p95/p99 и командой воспроизведения до любого изменения; якорь фальсифицируемости строки 9 теперь записываем. v1.6.0: performance-engineer лишён Write/Edit — то же выравнивание B-ревьюера; его собственное описание всегда говорило «передай coder для реализации» — marketplace#236) | RFC-012 — coder GREEN-phase discipline (FR-4) |
| **agents-domain** | **1.2.0** (README фиксирует скоуп-решение: сознательно не forgeplan-aware — было незадокументированной дырой, marketplace#236) | — |
| **agents-pro** | **1.27.0** (по итогам независимого аудита CONCERNS: injection-analyst и pii-detector получили Prompt-defense-преамбулу, Identity & audit, Step 4.5 ground-truth, cite-directive ADR-013 и отдельную секцию HARD RULES; injection-analyst поднят на opus (рассуждение о враждебном вводе — класс security-reasoner); аллоулисты дополнены search/recall/mental_model_get. v1.24.0: (injection-analyst и pii-detector получили forgeplan-поверхность профиля B (get/list/new/update/link/validate/score/claim/release; activate/reason/claims/memory_retain запрещены пропуском) + переносимые HARD RULES дисциплины EVID в телах — вторая половина дефекта строк 8-9, найденного forensic-аудитом. v1.23.0: injection-analyst и pii-detector лишены Write/Edit — ревьюер с меткой B и правом правки исходников не ревьюер; метка была advisory, аллоулист теперь её подкрепляет (marketplace#236). v1.22.0: (guardian получил строку продуктовой дисциплины (ADR-023 D4): Standard+ PRD с пустыми Non-Goals или без пользовательской работы → CONCERNS — структурная проверка присутствия, не суждение о качестве открытия. v1.21.0: (в денилисте guardian наконец есть forgeplan_activate — тело трижды объявляло активацию запретной, а фронтматтер разрешал: гейт активации был единственным Profile B, структурно способным активировать (EVID-231). v1.20.0: (`artifact-author` больше не отправляет создание EPIC агенту, который EPIC не создаёт: `goal-planner` — потребитель EPIC (разбирает родителя на RFC), а не его автор; строка CREATE в матрице CRUD-R-A теперь называет, у каких kind есть специалист, а у каких четырёх нет — marketplace#234. v1.19.0: guardian читает NOTE «Gate override:» — до этого обход гейта записывался и никем не читался, то есть был почти бесплатным, marketplace#237. v1.18.1: шаблоны тела EVID у `artifact-reviewer` и `evidence-gatherer` переведены на две оси — marketplace#251. v1.17.0: три техники получили гейт у guardian — NFR, DDD и STRIDE, все уровня CONCERNS по образцу строки C4; artifact-reviewer требует NFR у PRD и сценарий у каждого Requirement в SPEC — marketplace#227, #232, #235. denylists are Claude Code-only — `disallowedTools` appears zero times in the OMP binary, so the activate invariant now lives in the agent bodies where it travels (marketplace#218)) | EPIC-003 guardian Step 5 rows + EPIC-002 smith B-orchestrator |
| **agents-github** | **1.3.0** (README фиксирует то же скоуп-решение — marketplace#236) | — |
| **agents-sparc** | **1.6.0** (pseudocode и refinement документируют модель ограничений: аллоулист без единого forgeplan-инструмента — гарантия сильнее денилиста; «нет денилиста» ≠ «неограничен» (marketplace#236). v1.5.1: (шаблон NFR в specification переведён на таблицу ID/Category/Requirement/Metric/Condition/Measurement — форма уже жила в 19 PRD, но нигде не была закреплена, marketplace#227. dropped the word "ALL" from the sparc-orchestrator phase-agent row — four of those agents ship inside agents-sparc itself, marketplace#215; denylists are Claude Code-only — `disallowedTools` appears zero times in the OMP binary, so the activate invariant now lives in the agent bodies where it travels (marketplace#218)) | SPARC instance #3 (RFC-016) — sparc-orchestrator → B-orchestrator + /sparc skill |
| **agents-tdd** | 0.4.0 | TDD instance #1 (RFC-012) — tdd-orchestrator + RED/GREEN agents + tdd-test-validator + fail-closed gate |
| **agents-bmad** | 0.4.0 | BMAD instance #2 (RFC-013) — bmad-orchestrator persona-walk + no-code-before-plan gate |
| **agents-canvas** | **1.4.0** (шаблоны тела EVID у `canvas-tester` и `canvas-storybook-validator` переведены на две оси — marketplace#251. v1.1.0: три команды `/canvas-*` снова грузятся с метаданными — marketplace#244) | CANVAS instance #5 (RFC-021 + RFC-022/ADR-015) — canvas-coordinator design→code master + tokens-before-code hook-gate + 7 role agents (incl. canvas-storybook-validator). **v1.0.0 STACK-AGNOSTIC (breaking)** — native single-framework generation; engine/framework/design-source/language = inputs resolved via Step 0 (detect → announce / force-ask); Lit/Web-Components + Spread/wrappers demoted to an optional out-of-default multi-framework path (ADR-016); Style-Dictionary one token-tool option; context7 mandatory; острый gate hardened for native layouts (per-framework guarded globs + state-schema migration + fail-safe stale-state substitution, canvas-gate.sh mechanism unchanged). **brand/style-agnostic** (designer Step 0 chooses + records the brand); v0.3.0 gate hardening — vision-first visual verdict, console-error gate, font-load assertion, data-state/interaction oracles, missing-master PROBLEM loop, master-anatomy completeness, coordinator-verified (optional-path) framework-porter ownership |
