...
## Plugin versions (catalog v1.131.0)
...
| **forgeplan-orchestra** | **1.5.0** (`/sync` field-write path fixed — the non-existent `set_fields` replaced by `update_entity`, `fields` passed as `{fieldUid, value}` with UIDs resolved via `list_fields`, `create_entity` moved to the `entities[]` form; marketplace#208) |
