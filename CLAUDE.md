...
| **fpf** | **1.6.0** (`/fpf-lookup` shipped — it was promised in 10 places across 6 docs and did not exist, while the LOOKUP behaviour it names was already built inside `/fpf`; marketplace#215) |
| **agentic-rag** | 1.1.1 |
...
| **forgeplan-map-pack** | **0.21.0** (denylists are Claude Code-only — `disallowedTools` appears zero times in the OMP binary, so the activate invariant now lives in the agent bodies where it travels (marketplace#218) — 6 of its 8 agents were relying on the denylist alone) |
